...
// Content style
var globalOptions = PExProject.getGlobalOptions();
var contentStyleOption = globalOptions.getOption("contentStyle");
var contentStyleOptionValue = (contentStyleOption != null) ? contentStyleOption.getValue() : "";

// "CMSIS", "Zephyr", "Zephyr_SDK" or "SDK"; SDK or Zephyr style can contain CMSIS access to registers
var codeStyle = (contentStyleOptionValue == "Zephyr_SDK" ? "Zephyr_SDK" : "Zephyr");
//var codeStyle = "CMSIS";

/*
* Check code style type
* codeStyleTemp - Current code style
* codeStyleType - CMSIS, SDK or Zephyr
*/
function isCodeStyleType(codeStyleTemp,codeStyleType) {
  let returnVal = false;
  switch (codeStyleType) {
    case "CMSIS":
      returnVal = (codeStyleTemp == "CMSIS") || (codeStyleTemp == "Zephyr");
      break;
    case "SDK":
      returnVal = (codeStyleTemp == "SDK") || (codeStyleTemp == "Zephyr_SDK");
      break;
    case "Zephyr":
      returnVal = (codeStyleTemp == "Zephyr") || (codeStyleTemp == "Zephyr_SDK");
      break;  
    default:
      returnVal = false;
      break;
...
// "SDK", "Zephyr" or "nonSDK"; It affects include (fsl_common.h, fsl_) generating
var projectType = (contentStyleOptionValue == "Zephyr_SDK" ? "SDK" : "Zephyr");
//var projectType = "";

// Project type is Zephyr
function isProjectTypeZephyr(projectTypeTemp) {
  let returnVal = (projectTypeTemp == "Zephyr");
  return returnVal;
}

// Enables/Disables adding dependencies.
//var enablePinsToolDependencies = true;
var enablePinsToolDependencies = false;
...
// Root register name of Interrupt Control Register
var gpioInterruptControlRegisterName = "ICR";
// Root register name of EFT Detect Interrupt Enable
var portInterruptControlRegisterName = "EDIER";
...
// Mask of pin interrupt bit-field in GPIO Interrupt Control Register.
...
// Root peripheral name of INPUTMUX peripheral
var inputMuxPeripheralName = "INPUTMUX0";
...
var ptxRegex = /^PIO[A-Z][0-9][0-9]?$/;
var portxRegex = /^PORT[0-9]_[0-9][0-9]?$/;
...
var driverConstants = {
    ...
    'PCR_PV_HIGH': 'kPORT_HighPullResistor',
    'PCR_PV_LOW': 'kPORT_LowPullResistor',
    'PCR_PS_UP': 'kPORT_PullUp',
    'PCR_PS_DOWN': 'kPORT_PullDown',
    'PCR_PE_ENABLED': 'kPORT_PullEnable',
    'PCR_PE_DISABLED': 'kPORT_PullDisable',
    'PCR_IBE_ENABLED': 'kPORT_InputBufferEnable',
    'PCR_IBE_DISABLED': 'kPORT_InputBufferDisable',
    'PCR_INV_NORMAL': 'kPORT_InputNormal',
    'PCR_INV_INVERT': 'kPORT_InputInvert',
...
function filterPrintFunctions(x) {
    return ((x.printFunction.name != 'printGpioPinInit') && (x.printFunction.name != 'printGPIOSetPinInterruptConfig'));
}

function filterPrintFunctionsPostReg(x) {
    return (x.printFunction.name == 'printGPIOSetPinInterruptConfig');
}
...
scriptApi.requireScript("zephyr_defines_objects.js");
...
if (registerDatabaseObject != null) {
    ...
    if (isCodeStyleType(codeStyle,"SDK")) {
        var loadXbar = false;
        var loadDmamux = false;
        var loadInputMux = false;
        ...
        for (var ip = 0; ip < peripherals.length; ip++) {
            ...
            if (peripheralName.indexOf(inputMuxPeripheralName) >= 0) {
                loadInputMux = true;
            }
        ...
        if (cpuVariant != null) {
            ...
            if (loadInputMux) {
                loadingPath = "scripts/pins/INPUTMUX.js";
                var inputMuxSource = PExProcessor.loadPersistentObject("inputmux_source", loadingPath);
                if (inputMuxSource == null) {
                    logMessage('There is problem with loading ' + loadingPath, false);
                }
            }
            if (loadTrgmux) {
...
var configurationStrategiesSorted = new Array(); // Sorted configuration strategies
...
for (var pc = 0; pc < allComponents.length; pc++) {
    ...
    if (funcNameItem != null) {
        ...
        configurationStrategiesSorted[pc] = new Object();
        configurationStrategiesSorted[pc].text = funcNameItem.getText();
        configurationStrategiesSorted[pc].index = pc;
        configurationStrategiesSorted[pc].yamlState = allComponents[pc].getYamlState()
    }
...
// Sorting of the configuration strategies
configurationStrategiesSorted.sort(function(fg1, fg2) 
                                    {   let fgName1 = fg1.text.toLowerCase() 
                                        let fgName2 = fg2.text.toLowerCase()
                                        return (fgName1 > fgName2 ? 1 : (fgName1 < fgName2 ? -1 : 0 )) });
...
function getPortInitialsStr(registerName) {
    return (getPortInitialStr(registerName) + "_" + getPortIndexStr(registerName));
}
...
var setPinConfigTable = {
    ...
    0x0004: {
        "0": {
            "state": "kPORT_LowPullResistor",
            "description": "Low internal pull resistor value is selected."
        },
        "1": {
            "state": "kPORT_HighPullResistor",
            "description": "High internal pull resistor value is selected."
        },
        "offset": "2",
        "configure": "optional",
        "setting": "pullValueSelect"
    }, //PV
    0x0008: {
        "0": {
    ...
    0x0010: {
        "0": {
            ...
            "description": "Passive input filter is disabled"
        },
        "1": {
            "state": "kPORT_PassiveFilterEnable",
            "description": "Passive input filter is enabled"
        },
        "offset": "4",
        "configure": "optional",
        "setting": "passiveFilterEnable"
    }, //PFE
    0x0020: {
        "0": {
            "state": "kPORT_OpenDrainDisable",
            "description": "Open drain output is disabled"
        },
        "1": {
            "state": "kPORT_OpenDrainEnable",
            "description": "Open drain output is enabled"
        },
        "offset": "5",
        "configure": "optional",
        "setting": "openDrainEnable"
    }, //ODE
    0x0040: {
        "0": {
...
setPinConfigTable[0x1000] = {
    "0": {
        "state": "kPORT_InputBufferDisable",
        "description": "Digital input disabled; it is required for analog functions"
    },
    "1": {
        "state": "kPORT_InputBufferEnable",
        "description": "Digital input enabled"
    },
    "offset": "12",
    "configure": "optional",
    "setting": "inputBuffer"
}; //IBE
setPinConfigTable[0x2000] = {
    "0": {
        "state": "kPORT_InputNormal",
        "description": "Digital input is not inverted"
    },
    "1": {
        "state": "kPORT_InputInvert",
...
setPinConfigTable[0x8000] = {
    ...
    "offset": "15",
    "configure": "optional",
    "setting": "lockRegister"
}; //LK

var setIrqGpioPinConfigTable = {
    "0" : {
        "state": "kGPIO_InterruptStatusFlagDisabled",
        "description": "Interrupt/DMA request is disabled"
    },
    "1" : {
        "state": "kGPIO_DMARisingEdge",
        "description": "DMA request on rising edge"
    },
    "2" : {
        "state": "kGPIO_DMAFallingEdge",
        "description": "DMA request on falling edge"
    },
    "3" : {
        "state": "kGPIO_DMAEitherEdge",
...
function printPortSetPinConfig(accessRegs) {
    ...
    if (!(localStructureName)) {
        localStructureName = 'port' + gpioNameIndex.toLowerCase() + '_pin' + coordinates + '_config' + (testMode ? pc : "");
        if (isNotBounded) {
            localStructureName = 'port' + gpioNameIndex.toLowerCase() + '_nbpin_config' + (testMode ? pc : "");
        }
    };
    ...
    for (var mask in setPinConfigTable) {
        ...
        if (((configTableMask.configure == "always") || (getBitwiseAnd(globalPcrMask, mask) > 0)) && (mask != pinIrqcBitMask)) {
            ...
            var codeLine = "." + configTableMask.setting + " = " + configTableMaskValue.state;
            if (mask != lastMask) {
                codeLine += ",";
            }
...
function enrolPortSetPinConfig(reg, writeFunctions, configurationConstantDefinitionList, pc) {
    ...
    var configurableMask = getBitwiseAnd(getBitwiseAnd(0x3FFF, dbRegister.getNonReservedMask()), nonReadOnlyMask);
    var writeFunc = new Object();
    if (registerName.indexOf(portControlRegisterName) >= 0) {
...
function enrolGpioPinInit(reg, writeFunctions, configurationConstantDefinitionList, pc) {
    ...
    if (registerName.indexOf(gpioDirectionRegisterName) >= 0) {
        ...
        if (gpioSignalConfiguration[pc][gpioIdx].states == null) {
            ...
            var pdoField = pdorReg.findBitFieldByName("PDO0");
            ...
            for (var i = 0; i < 32; i++) {
                gpioSignalConfiguration[pc][gpioIdx].states[i] = 0;
            }
        }
    ...
    if (gpioSignalConfiguration[pc] != null) {
        Object.keys(gpioSignalConfiguration[pc]).forEach(function(gpioNameIndex) {
            for (var i = 0; i < gpioSignalConfiguration[pc][gpioNameIndex].pins.length; i++) {
                ...
                if (!(localStructureName)) {
                    localStructureName = 'gpio' + gpioNameIndex.toLowerCase() + '_pin' + coordinates + '_config' + (testMode ? pc : "");
                    if (isNotBounded) {
                        localStructureName = 'gpio' + gpioNameIndex.toLowerCase() + '_nbpin_config' + (testMode ? pc : "");
                    }
                } else {
                    localStructureName += "_config" + (testMode ? pc : "");
                }
                ...
                var comment = 'Initialize GPIO functionality on pin PIO' + gpioNameIndex + '_' + gpioSignalConfiguration[pc][gpioNameIndex].pins[i] + ' ';
                if (isNotBounded) {
                    comment += '(n/b pin) ';
                } else {
                    comment += '(pin ' + coordinates + ') ';
...
function enrolPortEFTGPIOPinInterruptConfig(reg, writeFunctions, configurationConstantDefinitionList, pc) {
    var registerName = reg.registerName;
    var dbRegister = registerDatabaseObject.getRegisterByFullName(reg.registerName);
    var configurableMask = pinIrqcBitMask;
    var writeFunc = new Object();
    if (registerName.indexOf(gpioInterruptControlRegisterName) >= 0) {
        var setValMask = getBitwiseAnd(getBitwiseOr(reg.registerClrMask, reg.registerSetMask), configurableMask);
        if (getBitwiseXor(setValMask, configurableMask) == 0) {
            var defValue = getPortIndexStr(registerName);
            // registerConstantDefinition('PIN' + normalizeNumberStr(defValue) + '_IDX', 'PIN' + defValue + '_IDX', defValue, 'Pin number for pin ' + defValue + ' in a port', 'index', configurationConstantDefinitionList);
            writeFunc.printFunction = printGPIOSetPinInterruptConfig;
            writeFunc.accessRegisters = new Array();
            var accessRegister = new Object();
            accessRegister.register = reg;
            accessRegister.setValue = getBitwiseAnd(reg.registerSetMask, setValMask);
    ...
    if (registerName.indexOf(portInterruptControlRegisterName) >= 0) {
        var setValMask = getBitwiseOr(reg.registerClrMask, reg.registerSetMask);
        if (setValMask != 0) {
            var defValue = getPortIndexStr(registerName);
            writeFunc.printFunction = printPortEFTSetPinInterruptConfig;
            writeFunc.accessRegisters = new Array();
            var accessRegister = new Object();
            accessRegister.register = reg;
            accessRegister.setValue = getBitwiseAnd(reg.registerSetMask, setValMask);
            accessRegister.clrValue = getBitwiseAnd(reg.registerClrMask, setValMask);
            writeFunc.accessRegisters.push(accessRegister);
            var writeFunctionsIndex = writeFunctions.push(writeFunc) - 1;
            reg.registerClrMask = 0;
            reg.registerSetMask = 0;
...
function printGPIOSetPinInterruptConfig(accessRegs) {
    var accessRegister = accessRegs[0];
    var reg = accessRegister.register;
    var registerName = reg.registerName.replace(gpioPeripheralName,portPeripheralName).replace(gpioInterruptControlRegisterName,portControlRegisterName);
    var gpioNameIndex = getPortInitialsStr(registerName);
    var coordinates = getSelectedPinCoordinatesStr(registerName, portMuxMask);
    var configTableMask = setIrqGpioPinConfigTable;
    ...
    var comment = 'Interrupt configuration on GPIO' + gpioNameIndex;
    if (isNotBoundedPin(coordinates)) {
        comment += ' (n/b pin) ';
    } else {
        comment += ' (pin ' + coordinates + '): ';
    ...
    PExOut_gen('');
    if (getRoutedPinGpio(registerName) && getRoutedPinPin(registerName)) {
        PExOut_gen(cIndent4 + makeComment(comment) + '\n' + cIndent4 + 'GPIO_SetPinInterruptConfig(' + getRoutedPinGpio(registerName) + ', ' + getRoutedPinPin(registerName) + ', ' + configTableMaskValueState + ');');
    } else {
        PExOut_gen(cIndent4 + makeComment(comment) + '\n' + cIndent4 + 'GPIO_SetPinInterruptConfig(GPIO' + getPortInitialStr(registerName) + ', ' + getPortIndexStr(registerName) + 'U, ' + configTableMaskValueState + ');');
    }
    reg.writeFunctions = null;
}

function printPortEFTSetPinInterruptConfig(accessRegs) {
    var accessRegister = accessRegs[0];
    var reg = accessRegister.register;
    var registerName = reg.registerName;
    var gpioNameIndex = getPortInitialsStr(registerName);
    var comment = 'EFT detect interrupts configuration on PORT' + gpioNameIndex;
    PExOut_gen('');
    if (accessRegister.setValue > 0) {
        PExOut_gen(cIndent4 + makeComment(comment) + '\n' + cIndent4 + 'PORT_EnableEFTDetectInterrupts(PORT' + getPortInitialStr(registerName) + ', ' + getNumberConvertedToHex(accessRegister.setValue) + ');');
    }
    if (accessRegister.clrValue > 0) {
        PExOut_gen(((accessRegister.setValue == 0) ? cIndent4 + makeComment(comment) + '\n' : '') + cIndent4 + 'PORT_DisableEFTDetectInterrupts(PORT' + getPortInitialStr(registerName) + ', ' + getNumberConvertedToHex(accessRegister.clrValue) + ');');
    }
    reg.writeFunctions = null;
}
...
function printDmaMuxSetSource(accessRegs) {
    ...
    var requestSource = dmaRequestSource[peripheralName][source.toString()];
    ...
    if (selectedRequestSource != null) {
        ...
        PExOut_gen(cIndent4 + 'EDMA_SetChannelMux(DMA0, ' + channel + 'U, (uint8_t)' + selectedRequestSource.id + ');');
...
function enrolDmaMuxSetSource(reg, writeFunctions, configurationConstantDefinitionList, pc) {
    ...
    if ((dmaRequestSource != null) && (registerName.indexOf(dmaMuxPeripheralName) >= 0) && (registerName.indexOf(dmaMuxChannelConfigRegisterName) >= 0)) {
        ...
        if (getBitwiseXor(setValMask, configurableMask) == 0) {
            ...
            var index = channel.indexOf('_MUX');

            channel = channel.substr(0, index);

           // channel = channel.substring(registerName.indexOf(dmaMuxChannelConfigRegisterName) + dmaMuxChannelConfigRegisterName.length);
...
function printClockRegister(reg) {
    if (isCodeStyleType(codeStyle,"SDK")) { //MUX bity/ SDK funkce
...
function printRegister(reg) {
    if (isCodeStyleType(codeStyle,"SDK")) {
        ...
        if (writeFunctions != null) {
            writeFunctions = writeFunctions.filter(filterPrintFunctions).sort(sortPrintFunctions);
...
/**
 * Print a register post GPIO
 * reg - register defined by name, clr mask and set mask
 * return no value
 */
function printRegisterPost(reg) {
    if (isCodeStyleType(codeStyle,"SDK")) {
        var writeFunctions = reg.writeFunctions;
        if (writeFunctions != null) {
            writeFunctions = writeFunctions.filter(filterPrintFunctionsPostReg).sort(sortPrintFunctions);
            for (var wf = 0; wf < writeFunctions.length; wf++) {
                writeFunctions[wf].printFunction(writeFunctions[wf].accessRegisters);
            }
        }
                        ...
                        if (clockGateField != null) {
                            for (var gindex = 0; gindex < clockGateStates.length; gindex++) {
                                ...
                                if (aGState.getName() == "enabled") {
                                    if (isCodeStyleType(codeStyle,"CMSIS")) {
...
/**
 * Puts calling INPUTMUX_AttachSignal into output 
 * accessRegs - reference to registers to be configured with values
 * return no value; just prining into ouput
 */
function printAttachSignal(accessRegs) {
    var accessRegister = accessRegs[0];
    var reg = accessRegister.register;
    var registerName = reg.registerName;
    var dbPeripheralName = registerDatabaseObject.getPeripheralByFullName(registerName).getName();
    var setValue = accessRegister.setValue;
    var regIndex = accessRegister.regIndex;
    var regShortName = accessRegister.regShortName;
    var dbRegister = registerDatabaseObject.getRegisterByFullName(reg.registerName);
    var dbBitFields = dbRegister.getBitFields();
    // scriptApi.logWarning('+++++++ accessRegister: ' + accessRegister);
    // scriptApi.logWarning('+++++++ reg:            ' + reg);
    // scriptApi.logWarning('+++++++ registerName:   ' + registerName);
    // scriptApi.logWarning('+++++++ setValue:       ' + setValue);
    // scriptApi.logWarning('+++++++ regIndex:       ' + regIndex);
    var selectedConnection = inputMuxSource[regShortName][setValue][0];
    if (selectedConnection != null) {
        var comment = selectedConnection.description + " " + regIndex;
        PExOut_gen(cIndent4 + makeComment(comment));
        PExOut_gen(cIndent4 + 'INPUTMUX_AttachSignal(' + dbPeripheralName + ', ' + regIndex + 'U, ' + selectedConnection.id + ');');
    }
    reg.writeFunctions = null;
}

/**
 * Registers using SDK function INPUTMUX_AttachSignal 
 * reg - register defined by name, clr mask and set mask
 * writeFunctions - reference to writeFunctions array (collection of data for SDK write functions) 
 * configurationConstantDefinitionList - given configuration where constant definitions are added
 * return no value
 */
function enrolAttachSignal(reg, writeFunctions, configurationConstantDefinitionList, pc) {
    var registerName = reg.registerName;
    var dbRegister = registerDatabaseObject.getRegisterByFullName(reg.registerName);
    var inputMuxRegs = {
        "SCT0_INMUX": 0x7F,
        ...
        "QDC1_PHASEB": 0x3F,
        "QDC1_PHASEA": 0x3F,
        "FlexPWM0_SM0_EXTSYNC" : 0x3F,
        "FlexPWM0_SM1_EXTSYNC" : 0x3F,
        "FlexPWM0_SM2_EXTSYNC" : 0x3F,
        "FlexPWM0_SM3_EXTSYNC" : 0x3F,
        "FlexPWM0_SM0_EXTA" : 0x3F,
        "FlexPWM0_SM1_EXTA" : 0x3F,
        "FlexPWM0_SM2_EXTA" : 0x3F,
        "FlexPWM0_SM3_EXTA" : 0x3F,
        "FlexPWM0_EXTFORCE" : 0x3F,
        "FlexPWM0_FAULT0" : 0x3F,
        "FlexPWM0_FAULT1" : 0x3F,
        "FlexPWM0_FAULT2" : 0x3F,
        "FlexPWM0_FAULT3" : 0x3F,
        "FlexPWM1_SM0_EXTSYNC" : 0x3F,
        "FlexPWM1_SM1_EXTSYNC" : 0x3F,
        "FlexPWM1_SM2_EXTSYNC" : 0x3F,
        "FlexPWM1_SM3_EXTSYNC" : 0x3F,
        "FlexPWM1_SM0_EXTA" : 0x3F,
        "FlexPWM1_SM1_EXTA" : 0x3F,
        "FlexPWM1_SM2_EXTA" : 0x3F,
        "FlexPWM1_SM3_EXTA" : 0x3F,
        "FlexPWM1_EXTFORCE" : 0x3F,
        "FlexPWM1_FAULT0" : 0x3F,
        "FlexPWM1_FAULT1" : 0x3F,
        "FlexPWM1_FAULT2" : 0x3F,
        "FlexPWM1_FAULT3" : 0x3F,
        "FlexPWM1_FAULT" : 0x3F,
        "FlexPWM1_FORCE" : 0x3F,
        "EVTG_TRIG" : 0x3F,
        "USBFS_TRIG" : 0xF,
    ...
    var writeFunc = new Object();
    if ((inputMuxSource != null) && (registerName.indexOf(inputMuxPeripheralName) >= 0)) {
        for (var inputMuxReg in inputMuxRegs) {
            if (registerName.indexOf(inputMuxReg) >= 0) {
                var regIndex = registerName.substring(registerName.indexOf(inputMuxReg) + inputMuxReg.length);
                if (regIndex == "") regIndex = 0;
                //registerConstantDefinition(inputMuxReg + normalizeNumberStr(regIndex) + '_IDX', inputMuxReg + regIndex + '_IDX', regIndex, 'Inputmux ' + inputMuxReg + ' register index ' + regIndex, 'index', configurationConstantDefinitionList);
                var configurableMask = inputMuxRegs[inputMuxReg];
                var setValMask = getBitwiseAnd(getBitwiseOr(reg.registerClrMask, reg.registerSetMask), configurableMask);
                if (getBitwiseXor(setValMask, configurableMask) == 0) {
                    var setValue = getBitwiseAnd(reg.registerSetMask, setValMask);

                    writeFunc.printFunction = printAttachSignal;
                    writeFunc.accessRegisters = new Array();
                    var accessRegister = new Object();
                    accessRegister.register = reg;
                    accessRegister.setValue = setValue;
                    accessRegister.configurableMask = configurableMask;
                    accessRegister.regIndex = regIndex;
                    accessRegister.regShortName = inputMuxReg;
                    writeFunc.accessRegisters.push(accessRegister);
                    var writeFunctionsIndex = writeFunctions.push(writeFunc) - 1;
                    reg.registerClrMask = getBitwiseAnd(reg.registerClrMask, getBitwiseNeg32(configurableMask));
                    reg.registerSetMask = getBitwiseAnd(reg.registerSetMask, getBitwiseNeg32(configurableMask));
                    ...
                    reg.writeFunctions.push(writeFunctions[writeFunctionsIndex]);
                    include_fsl_inputmux['present'] = true;
                    include_fsl_inputmux['componentsAndCores'][pc] = componentCoreIds[!testMode ? pc : 0];   // Save componentCoreIds[0] in testMode instead of current "pc / test configuration" normally used as index for current component 'coreId' assignment
                }
            }
        }
    }
...
function enrolSdkFunctions(pc, ccd) {
    ...
    for (var r in configurationRegisterList) {
        enrolPortEFTGPIOPinInterruptConfig(configurationRegisterList[r], configurationWriteClockFunctions, configurationConstantDefinitionList, pc);
        ...
        enrolAttachSignal(configurationRegisterList[r], configurationWriteClockFunctions, configurationConstantDefinitionList, pc);
    }
}
...
function processRegisterConfigurationSequence(coreId) {
    ...
    */
    ...
    if (isCodeStyleType(codeStyle,"Zephyr")) {
      for (var pc = 0; pc < allComponents.length; pc++) { // Pin configuration represented by tables in the UI of the tool
        currentComponent = pc;
        if (coreId == componentCoreIds[pc]) {
          enrolDTSPinCfg(pc,pc);
        }
      }
    }
    ...
    if (isCodeStyleType(codeStyle,"SDK")) {
        for (var pc = 0; pc < allComponents.length; pc++) { // Pin configuration represented by tables in the UI of the tool
            currentComponent = pc;
            if ((coreId == componentCoreIds[pc]) && ((Object.keys(registerList[pc]) != "") || !isProjectTypeZephyr(projectType))) {
...
function printOneRegisterConfiguration(configuration) {
    ...
    /* ***** GPIO pin initialization code for given pin IO direction related features (output initial state) based on SDK API ***** */
    printGpioPinInit(configuration);

    /* ***** Pin muxing, functional property Register configurations ***** */
    var configurationRegisterList = registerList[configuration];
    for (var r in configurationRegisterList) {
        printRegisterPost(configurationRegisterList[r]);
    }
}
...
function printGpioIdentifierDefine(identifierDescriptor) {
    ...
    for (var identifierIndex in identifierArray) {
        ...
        // null when "No init" option is selected or when input direction is selected
        if (identifierDescriptor.initState != null) {
            PExOut_gen(getConstantDefinitionStr(
                identifierBody + "_INIT_GPIO_VALUE",
                identifierDescriptor.initState,
...
function getPortString(pinItem) {
    ...
    for (var stepIndex = 0; stepIndex < configurationSteps.length; stepIndex++) {
        ...
        for (var configurationIndex = 0; configurationIndex < registersConfigurations.length; configurationIndex++) {
            ...
            var nameSubMatches = registerName.match(/PORT([0-9])_PCR(\d+)/);
            if (nameSubMatches != null) {
                if (nameSubMatches.length == 3) {
                    PinIdentifier = "PORT" + nameSubMatches[1] + "_" + nameSubMatches[2];
                    break;
                }
            }
        }
...
function printIdentifierDefines(configuration) {
    ...
    for (var i = 0; i < numOfItems; i++) {
        ...
        var dirItem = allComponents[configuration].findItemBySymbol("direction" + i);
        var initState = allComponents[configuration].findItemBySymbol("gpio_init_state" + i);
        var interruptItem = allComponents[configuration].findItemBySymbol("gpio_per_interrupt" + i);
        var interruptEFTItem = allComponents[configuration].findItemBySymbol("eft_interrupt" + i);
        var coordinate = pinItem.getSelectedPinCoordinates();
        var noCoordinate = (coordinate == null) || (coordinate == "");
        if ((peripheralItem != null) && (!(Contains(peripheralItem.getTextValue(), 'GPIO')) || ((interruptItem != null) && Contains(interruptItem.getTextValue(), 'kGPIO_') && !(Contains(interruptItem.getTextValue(), 'Disabled'))) || ((interruptEFTItem != null) && Contains(interruptEFTItem.getTextValue(), 'enable')))) {
            pushDependency(dependencyHelper, createDependency(peripheralItem.getTextValue(), 'Peripheral', configuration));
        }
        if ((pinItem != null) && (noCoordinate)) {
            var pinItemText = pinItem.getTextValue().toUpperCase().replace('EVTG_','EVTG0_');
            var internalSignalPeripheral = ((pinItemText.split('_')[0].length > 0) ? pinItemText.split('_')[0] : null);
            if ((internalSignalPeripheral != null) &&  (Contains(internalSignalPeripheral, "AOI") || Contains(internalSignalPeripheral, "EVTG"))) {
                pushDependency(dependencyHelper, createDependency(internalSignalPeripheral, 'Peripheral', configuration), componentCoreIds[configuration]);
            }
        }
        if (pinItem != null && peripheralItem != null && signalItem != null && identifierItem != null) {
            if (pinItem.getError() == null && peripheralItem.getError() == null && signalItem.getError() == null && identifierItem.getError() == null) {
                ...
                if (identifierText != "") {
                    ...
                    if (identifierDescriptor.direction == "OUTPUT") {
                        if (initState.getTextValue() == "true") {
                            identifierDescriptor.initState = "1U";
                        } else if (initState.getTextValue() == "false") {
                            identifierDescriptor.initState = "0U";
                        } else {
                            identifierDescriptor.initState = null;
                        }
                    } else {
                        identifierDescriptor.initState = null;
                    }
...
function createDependency(rId, rType, component) {
    ...
    if (rType == 'Peripheral') {
        var resId = rId;
        var desc = "Peripheral " + rId + " signals are routed in the Pins Tool, but the peripheral is not initialized in the Peripherals Tool.";
        var level = 1;
...
function pushDependency(dependencyHelper, dependency) {
    if (enablePinsToolDependencies && !(dependencyPushed(dependency))) {
...
function registerDependencies() {
    if (projectType == "SDK") {
        ...
        if (include_fsl_inputmux['present']) {
            for (var coreAndComponent in include_fsl_inputmux['componentsAndCores']) {
                pushDependency(dependencyHelper, createDependency('inputmux', 'SWComponent', coreAndComponent), include_fsl_inputmux['componentsAndCores'][coreAndComponent]);
            }
        }
    }
}
...
function printIncludes() {
    if (projectType == "SDK") {
        ...
        if (include_fsl_dmamux['present']) {
            PExOut_gen('#include \"fsl_edma.h\"');
        }
        if (include_fsl_trgmux['present']) {
            PExOut_gen('#include \"fsl_trgmux.h\"');
        }
        if (include_fsl_inputmux['present']) {
            PExOut_gen('#include \"fsl_inputmux.h\"');
        }
    } else {
        if (isProjectTypeZephyr(projectType)) {
            PExOut_gen('#include <zephyr/init.h>');
        }
        PExOut_gen('#include "fsl_device_registers.h"');
    }
}
...
function isEnabledCore(coreId) {
    var coreList = JSON.parse(coreListTxt);
    // To avoid possible undefined object access, first check if the coreList[coreId] defined, before getting coreId 'enabled' status.
    return ((coreList[coreId] != undefined) ? coreList[coreId]['enabled'] : false);
}
...
function printInitFunction(initFunctions) {
    if (isProjectTypeZephyr(projectType)) {
        ...
        PExOut_gen(' *');
        PExOut_gen(' * Function Name : board_init_boot_pins');
        PExOut_gen(' * Description   : Calls initialization functions.');
        PExOut_gen(' *');
        PExOut_gen(' * END ' + makeString('*', 112) + '/');
        PExOut_gen('static int board_init_boot_pins(void)');
        PExOut_gen('{');
...
function getInitFunctions() {
    ...
    for (var pc = 0; pc < allComponents.length; pc++) {
        ...
        if ((functionJSON['callFromInitBoot']) && (isEnabledCore(functionJSON['coreID'])) && ((Object.keys(registerList[pc]) != "") || !isProjectTypeZephyr(projectType))) {
...
function printDriver() {
    ...
    if (isEnabledCore(coreId)) {
        ...
        if (isProjectTypeZephyr(projectType)) {
            PExOut_gen('');
            PExOut_gen('SYS_INIT(board_init_boot_pins, PRE_KERNEL_1, 0);');
        }
        PExOut_gen('/' + makeString('*', 119));
        PExOut_gen(' * EOF');
        PExOut_gen(' ' + makeString('*', 118) + '/');
        
        // Include Zephyr pins print code
        if (isCodeStyleType(codeStyle,"Zephyr")) {
            scriptApi.requireScript("zephyr_pins_print_code.js");
        }
    }
    }
}
