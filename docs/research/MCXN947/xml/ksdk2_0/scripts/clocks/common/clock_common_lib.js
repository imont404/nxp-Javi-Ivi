...
ClockCodeGen = function() {
  ...
  this.makePrefix = function (name) {
    name = name.replace(/[^\w\d]/g, "_");
    // remove all underscore character from the end of the prefix
  ...
  // prefix name for defines and global variables
  this.getPrefixID = function(clkconfig) {
      return this.makePrefix(clkconfig.getIdPrefix());
  }
