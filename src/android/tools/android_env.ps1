...
$script:NxpCupAndroidToolsRoot = if (-not [string]::IsNullOrWhiteSpace($env:NXPC_ANDROID_TOOLS_DIR)) {
    $env:NXPC_ANDROID_TOOLS_DIR
} else {
    Join-Path $script:NxpCupAndroidRepoRoot "out\toolchains\android"
}
