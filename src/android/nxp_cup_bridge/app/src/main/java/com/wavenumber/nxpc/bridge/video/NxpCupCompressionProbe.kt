...
enum class NxpCupCompressionMode(val wireName: String) {
    JPEG("jpeg"),
    H264("h264");

    companion object {
        fun parse(value: String?): NxpCupCompressionMode? =
            entries.firstOrNull { it.wireName.equals(value, ignoreCase = true) }
    }
}

data class NxpCupCompressionSnapshot(
    val mode: NxpCupCompressionMode,
    val state: String,
    val encoder: String,
...
data class NxpCupJpegFrameView(
    val frameId: Long,
    val width: Int,
    val height: Int,
    val capturedNs: Long,
...
data class NxpCupH264FormatView(
    val csd0: ByteArray,
    val csd1: ByteArray,
)

data class NxpCupH264AccessUnitView(
    val frameId: Long,
    val capturedNs: Long,
...
class NxpCupCompressionProbe(
    private val mode: NxpCupCompressionMode,
    private val jpegQuality: Int = 70,
    private val h264Bitrate: Int = 750_000,
    private val onJpegFrame: ((NxpCupJpegFrameView) -> Unit)? = null,
    private val onH264Format: ((NxpCupH264FormatView) -> Unit)? = null,
    private val onH264AccessUnit: ((NxpCupH264AccessUnitView) -> Unit)? = null,
    private val onSnapshot: (NxpCupCompressionSnapshot) -> Unit,
) : AutoCloseable {
    companion object {
    ...
    private val worker = Thread(::run, "nxpc-${mode.wireName}-probe").also { it.start() }
    ...
    fun offerFrame(frame: NxpCupVideoFrame) {
    ...
    private fun run() {
        ...
        try {
            when (mode) {
                NxpCupCompressionMode.JPEG -> runJpeg(measurements)
                NxpCupCompressionMode.H264 -> runH264(measurements)
            }
    ...
    private fun runH264(measurements: Measurements) {
        val capability = NxpCupCodecInventory.encoders().firstOrNull {
            it.hardwareAccelerated && !it.name.endsWith(".secure") && it.name.startsWith("c2.")
        } ?: NxpCupCodecInventory.encoders().firstOrNull {
        ...
        val format = MediaFormat.createVideoFormat(NxpCupCodecInventory.MIME_TYPE, WIDTH, HEIGHT).apply {
        ...
        measurements.encoder = "${capability.name} ${NxpCupCodecInventory.colorName(colorFormat)} bitrate=$h264Bitrate"
            ...
            while (true) {
                ...
                if (outputIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    if (csd0 != null && csd1 != null) {
                        onH264Format?.invoke(NxpCupH264FormatView(csd0, csd1))
                    }
                    continue
                }
