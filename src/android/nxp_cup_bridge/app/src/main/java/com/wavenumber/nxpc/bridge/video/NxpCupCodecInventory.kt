...
data class NxpCupEncoderCapability(
    val name: String,
...
object NxpCupCodecInventory {
    const val MIME_TYPE = "video/avc"

    fun encoders(): List<NxpCupEncoderCapability> =
        MediaCodecList(MediaCodecList.ALL_CODECS).codecInfos
            .asSequence()
            .filter { it.isEncoder && it.supportedTypes.any { type -> type.equals(MIME_TYPE, true) } }
            .mapNotNull(::capability)
            .sortedWith(
                compareByDescending<NxpCupEncoderCapability> { it.hardwareAccelerated }
                    .thenBy { it.name },
            )
            .toList()

    fun logLine(codec: NxpCupEncoderCapability): String =
    ...
    private fun capability(info: MediaCodecInfo): NxpCupEncoderCapability? = try {
