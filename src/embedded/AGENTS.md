...
Build with `.\src\embedded\build.ps1`. Students flash with Ozone; tools under
`tools` are maintainer-only. Keep the Rev A competition configuration as the
default and keep diagnostics explicit.
