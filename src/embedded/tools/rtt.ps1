param(
    ...
    [int]$SpeedKHz = 4000,
    # See flash.ps1 / tools\lib\jlink_common.ps1. Empty resolves automatically.
    [string]$UsbSerial = "",
    [int]$Channel = 0
)
...
$NmPath = Resolve-NxpCupArmTool -RepoRoot $repoRoot -ToolName "arm-none-eabi-nm" -ConfiguredPath $NmPath

if (-not (Test-Path -LiteralPath $axfFile)) {
    throw "Firmware not found: $axfFile. Build first with .\src\embedded\build.ps1"
}

if ($Backend -eq "PyLink") {
    $monitor = Join-Path $repoRoot "scripts\tools\rtt_monitor.py"
