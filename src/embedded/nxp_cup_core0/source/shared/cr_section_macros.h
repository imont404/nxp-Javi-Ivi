/* VENDORED 2026-07-25 (cmake-build-and-toolchain/break-ide-dependencies).
 *
 * Copied verbatim from the MCUXpresso 25.6 install:
 *   ide/plugins/com.nxp.mcuxpresso.tools.win32_.../tools/arm-none-eabi/include/
 *
 * This header ships only with the IDE, but the firmware depends on it for
 * memory placement - __BSS(SRAM_H) holds the EZH program image and
 * __BSS(FRAME_BUFFERS) holds the camera frame buffers. Without it vendored,
 * a build using the standalone Arm GNU toolchain cannot resolve the include.
 *
 * Do not edit. If MCUXpresso ships a newer version, re-copy and diff.
 */
