...
#error CONFIG__DISPLAY_ENABLE must be 0 or 1.
    ...
    (CONFIG__DISPLAY_TEST_MODE || CONFIG__DISPLAY_TE_ENABLE ||                                     \
     CONFIG__DISPLAY_PARALLEL_BITBANG_TEST_MODE || CONFIG__DISPLAY_TIMING_DIAG_ENABLE ||           \
 ...
 *   P3_20_A3 PWM1
 * submodule 3, channel A, alt5 - Rev B candidate (shield J1.5)
 */
...
#if CONFIG__SERVO_PWM_OUTPUT != SERVO_PWM_OUTPUT_P2_3_B2 &&                                        \
    CONFIG__SERVO_PWM_OUTPUT != SERVO_PWM_OUTPUT_P3_20_A3
 ...
 * Instruments nxpc_display__write with the cycle counter and reports where the frame
 * dump time
 * goes. Measurement only - it changes no behavior - but it costs a few cycles per phase and prints
 * over RTT, so it stays off in the competition
 * image.
 */
 ...
 *
 * Drives P4_1 high for the whole per-frame LCD dump - both display writes -
 * and low between.
 * Pulse width is the total dump time, pulse period is the frame period, both read directly off a
 * scope.
 *
...
/* Report once per this many display writes. Two calls per frame. */
 ...
 * backend. It was
 * bench-proven with the EZH camera, SPI LCD, and USB telemetry.
 */
