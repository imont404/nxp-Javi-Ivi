...
AT_NONCACHEABLE_SECTION_INIT(static lpspi_master_edma_handle_t g_m_edma_handle) = {0};
static edma_handle_t lpspiEdmaMasterRxRegToRxDataHandle;
static edma_handle_t lpspiEdmaMasterTxDataToTxRegHandle;
static volatile bool isTransferCompleted = true;
...
static void lpspi1__transfer_callback(LPSPI_Type *base, lpspi_master_edma_handle_t *handle,
 ...
 */
static void lpspi1_set_frame_size(uint8_t transaction_bits)
{
    lpspi1_wait_idle();
 ...
 * clock settings
 * are documented beside CONFIG__DISPLAY_SPI_PLLCLKDIV.
 */
static uint32_t lpspi1_get_actual_sck_hz(uint32_t *src_hz_out, uint8_t *sckdiv_out,
                                         uint8_t *prescale_out)
{
    ...
    if (sckdiv_out != NULL)
    {
        *sckdiv_out = (uint8_t)sckdiv;
    }
    if (prescale_out != NULL)
    {
...
void lpspi1_report_clock(void)
{
    ...
    DEBUG("lpspi1 clock src=%luHz sckdiv=%u prescale=%u actual=%luHz requested=%luHz\r\n",
          (unsigned long)src, (unsigned)sckdiv, (unsigned)prescale, (unsigned long)sck,
          (unsigned long)CONFIG__TRANSFER_BAUDRATE);
    DEBUG("lpspi1 raw CCR=%08lX TCR=%08lX CFGR1=%08lX\r\n", (unsigned long)LPSPI1->CCR,
          (unsigned long)LPSPI1->TCR, (unsigned long)LPSPI1->CFGR1);
}
...
{
    return (uint8_t)(((LPSPI1->TCR & LPSPI_TCR_FRAMESZ_MASK) >> LPSPI_TCR_FRAMESZ_SHIFT) + 1U);
}
...
void lpspi1_init(uint8_t transaction_bits)
{
    ...
    EDMA_CreateHandle(&(lpspiEdmaMasterRxRegToRxDataHandle), DMA0, MASTER_DMA_RX_CHANNEL);
    EDMA_CreateHandle(&(lpspiEdmaMasterTxDataToTxRegHandle), DMA0, MASTER_DMA_TX_CHANNEL);
    EDMA_SetChannelMux(DMA0, MASTER_DMA_TX_CHANNEL, DEMO_LPSPI_TRANSMIT_EDMA_CHANNEL);
    EDMA_SetChannelMux(DMA0, MASTER_DMA_RX_CHANNEL, DEMO_LPSPI_RECEIVE_EDMA_CHANNEL);

    LPSPI_MasterTransferCreateHandleEDMA(LPSPI1, &g_m_edma_handle, lpspi1__transfer_callback, NULL,
...
void lpspi1_transfer_block(const void *block, uint32_t block_size)
{
    ...
    masterXfer.txData = (uint8_t *)block;
