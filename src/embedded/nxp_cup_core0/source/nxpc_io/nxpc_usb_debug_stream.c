...
#error NXP Cup USB debug stream expects the MCXN947 HS EHCI device controller.
#endif

#define NXPC_USB_CONTROLLER_ID kUSB_ControllerEhci0
#define NXPC_USB_INTERRUPT_PRIORITY (3U)
#define NXPC_USB_DATA_BUFF_SIZE HS_CDC_VCOM_BULK_OUT_PACKET_SIZE

#define NXPC_USB_STREAM_FRAME_WIDTH ((uint32_t)CONFIG__CAMERA_RESOLUTION_X)
#define NXPC_USB_STREAM_FRAME_HEIGHT ((uint32_t)CONFIG__CAMERA_RESOLUTION_Y)
#define NXPC_USB_STREAM_FRAME_BYTES                                                                \
    (NXPC_USB_STREAM_FRAME_WIDTH * NXPC_USB_STREAM_FRAME_HEIGHT * 2U)
#define NXPC_USB_STREAM_TX_BYTES (16U * 1024U)
#define NXPC_USB_STREAM_DATA_OFFSET                                                                \
    (NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_RUI_WRITE_FRAME_BUFFER_RAW_HEADER_BYTES)
#define NXPC_USB_STREAM_PAYLOAD_MAX_BYTES (NXPC_USB_STREAM_TX_BYTES - NXPC_USB_STREAM_DATA_OFFSET)
#define NXPC_USB_STREAM_STATS_INTERVAL_CAMERA_FRAMES (24U)
#define NXPC_USB_STREAM_STATS_INTERVAL_SYNTHETIC_FRAMES (0U)
#define NXPC_USB_CONTROL_RX_BYTES (1024U)
#define NXPC_USB_CONTROL_MAX_PAYLOAD_BYTES (128U)
#define NXPC_USB_CONTROL_RESPONSE_PAYLOAD_BYTES (NXPC_DBG_CONTROL_HELLO_RESPONSE_BYTES)
#define NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH (4U)
#define NXPC_USB_TX_CONTROL_BURST_MAX (4U)
#define NXPC_USB_LOG_QUEUE_DEPTH (8U)
...
#define NXPC_USB_CONTROL_CAPABILITIES                                                              \
    (NXPC_DBG_CAPABILITY_FRAMED_CONTROL | NXPC_DBG_CAPABILITY_CAMERA_FRAMES |                      \
     NXPC_DBG_CAPABILITY_SYNTHETIC_FRAMES | NXPC_DBG_CAPABILITY_STREAM_STATS |                     \
     NXPC_DBG_CAPABILITY_LOG_TEXT | NXPC_DBG_CAPABILITY_NAMED_TELEMETRY |                          \
     NXPC_DBG_CAPABILITY_ENTER_ISP | NXPC_DBG_CAPABILITY_SYSTEM_ACTIONS)
#define NXPC_USB_CONTROL_SUPPORTED_CHANNELS                                                        \
    (NXPC_DBG_CHANNEL_FRAMES | NXPC_DBG_CHANNEL_STATS | NXPC_DBG_CHANNEL_LOGS |                    \
     NXPC_DBG_CHANNEL_TELEMETRY)

#define NXPC_USB_LINE_CODING_SIZE (0x07U)
#define NXPC_USB_LINE_CODING_DTERATE (115200U)
#define NXPC_USB_LINE_CODING_CHARFORMAT (0x00U)
#define NXPC_USB_LINE_CODING_PARITYTYPE (0x00U)
#define NXPC_USB_LINE_CODING_DATABITS (0x08U)

#define NXPC_USB_COMM_FEATURE_DATA_SIZE (0x02U)
#define NXPC_USB_STATUS_ABSTRACT_STATE (0x0000U)
#define NXPC_USB_COUNTRY_SETTING (0x0000U)

#define NXPC_USB_NOTIF_PACKET_SIZE (0x08U)
#define NXPC_USB_UART_BITMAP_SIZE (0x02U)
#define NXPC_USB_NOTIF_REQUEST_TYPE (0xA1U)

typedef struct nxpc_usb_cdc_vcom
{
...
typedef struct nxpc_usb_cdc_acm_info
{
    uint8_t serialStateBuf[NXPC_USB_NOTIF_PACKET_SIZE + NXPC_USB_UART_BITMAP_SIZE];
    uint8_t dtePresent;
    uint16_t breakDuration;
    uint8_t dteStatus;
    uint8_t currentInterface;
    uint16_t uartState;
} nxpc_usb_cdc_acm_info_t;

typedef enum nxpc_usb_debug_stream_source
{
    NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA = NXPC_DBG_STREAM_SOURCE_CAMERA,
    NXPC_USB_DEBUG_STREAM_SOURCE_SYNTHETIC = NXPC_DBG_STREAM_SOURCE_SYNTHETIC,
} nxpc_usb_debug_stream_source_t;

typedef struct nxpc_usb_control_response
{
    uint32_t msgId;
    ...
    uint8_t payload[NXPC_USB_CONTROL_RESPONSE_PAYLOAD_BYTES];
} nxpc_usb_control_response_t;

typedef struct nxpc_usb_log_record
{
    ...
    char category[NXPC_USB_LOG_CATEGORY_MAX_BYTES];
    char text[NXPC_USB_LOG_TEXT_MAX_BYTES];
} nxpc_usb_log_record_t;

typedef struct nxpc_usb_telemetry_scalar
{
    uint32_t timestampMs;
    uint32_t sampleId;
    uint32_t valueBits;
...
static void nxpc_usb_debug_stream__schedule_tx(void);
static void nxpc_usb_debug_stream__handle_received_data(const uint8_t *data, uint32_t length);
static usb_status_t nxpc_usb_debug_stream__cdc_callback(class_handle_t handle, uint32_t event,
                                                        void *param);
static usb_status_t nxpc_usb_debug_stream__device_callback(usb_device_handle handle, uint32_t event,
                                                           void *param);

USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_lineCoding[NXPC_USB_LINE_CODING_SIZE] = {
    (NXPC_USB_LINE_CODING_DTERATE >> 0U) & 0xFFU,
    (NXPC_USB_LINE_CODING_DTERATE >> 8U) & 0xFFU,
    (NXPC_USB_LINE_CODING_DTERATE >> 16U) & 0xFFU,
    (NXPC_USB_LINE_CODING_DTERATE >> 24U) & 0xFFU,
    NXPC_USB_LINE_CODING_CHARFORMAT,
    NXPC_USB_LINE_CODING_PARITYTYPE,
    NXPC_USB_LINE_CODING_DATABITS,
};

USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_abstractState[NXPC_USB_COMM_FEATURE_DATA_SIZE] = {
    (NXPC_USB_STATUS_ABSTRACT_STATE >> 0U) & 0xFFU,
    (NXPC_USB_STATUS_ABSTRACT_STATE >> 8U) & 0xFFU,
};

USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_countryCode[NXPC_USB_COMM_FEATURE_DATA_SIZE] = {
    (NXPC_USB_COUNTRY_SETTING >> 0U) & 0xFFU,
    (NXPC_USB_COUNTRY_SETTING >> 8U) & 0xFFU,
};

USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE) static nxpc_usb_cdc_acm_info_t s_usbCdcAcmInfo;
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_currRecvBuf[NXPC_USB_DATA_BUFF_SIZE];
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_streamTxBuf[NXPC_USB_STREAM_TX_BYTES];

static nxpc_usb_cdc_vcom_t s_cdcVcom;
...
static nxpc_usb_debug_stream_source_t s_streamSource;
...
static uint8_t s_controlRxBuffer[NXPC_USB_CONTROL_RX_BYTES];
static nxpc_usb_control_response_t s_controlResponseQueue[NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH];
...
static nxpc_usb_log_record_t s_logQueue[NXPC_USB_LOG_QUEUE_DEPTH];
...
static nxpc_usb_telemetry_scalar_t s_telemetryQueue[NXPC_USB_TELEMETRY_QUEUE_DEPTH];
...
static usb_device_class_config_struct_t s_cdcAcmConfig[1] = {{
    nxpc_usb_debug_stream__cdc_callback,
    0,
...
static usb_device_class_config_list_struct_t s_cdcAcmConfigList = {
    s_cdcAcmConfig,
    nxpc_usb_debug_stream__device_callback,
    1,
};

#if CONFIG__USB_DEBUG_PROFILE_ENABLE
static void nxpc_usb_debug_stream__profile_service(uint32_t startCycles, uint8_t open)
{
    ...
    {
        averageCycles =
            (s_profileValidSamples != 0U) ? (s_profileTotalCycles / s_profileValidSamples) : 0U;
        (void)DEBUG(
            "nxpc_usb_profile: calls=%u open=%u avg_cycles=%u max_cycles=%u reset_samples=%u\r\n",
            s_profileCalls, s_profileOpenCalls, averageCycles, s_profileMaxCycles,
            s_profileInterruptedSamples);
        s_profileCalls = 0U;
...
static void nxpc_usb_debug_stream__fill_static_payload(void)
{
    for (uint32_t i = 0U; i < NXPC_USB_STREAM_PAYLOAD_MAX_BYTES; i += 2U)
    {
        uint32_t pixel = i >> 1U;
        ...
        s_streamTxBuf[NXPC_USB_STREAM_DATA_OFFSET + i] = (uint8_t)(rgb & 0xFFU);
        s_streamTxBuf[NXPC_USB_STREAM_DATA_OFFSET + i + 1U] = (uint8_t)((rgb >> 8U) & 0xFFU);
    }
}

static uint32_t nxpc_usb_debug_stream__stats_interval_frames(void)
{
    if (s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_SYNTHETIC)
    {
        return NXPC_USB_STREAM_STATS_INTERVAL_SYNTHETIC_FRAMES;
    }

    return NXPC_USB_STREAM_STATS_INTERVAL_CAMERA_FRAMES;
}

static void nxpc_usb_debug_stream__mark_frame_completed(void)
{
    uint32_t interval;
    ...
    interval = nxpc_usb_debug_stream__stats_interval_frames();
...
static uint8_t nxpc_usb_debug_stream__submit_packet(uint32_t packetBytes)
{
    ...
    error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_IN_ENDPOINT,
                                 s_streamTxBuf, packetBytes);
...
static uint8_t nxpc_usb_debug_stream__send_stats_report(void)
{
    nxpc_dbg_packet_header_t *packetHeader;
    nxpc_dbg_stats_report_t *stats;
    uint32_t streamFlags = 0U;
    uint32_t packetBytes = NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_STATS_REPORT_BYTES;
    ...
    {
        streamFlags |= NXPC_DBG_STATS_FLAG_STREAM_ENABLED;
    }
    if (s_streamTxBusy != 0U)
    {
        streamFlags |= NXPC_DBG_STATS_FLAG_TX_BUSY;
    }
    if (s_streamFrameActive != 0U)
    {
        streamFlags |= NXPC_DBG_STATS_FLAG_FRAME_ACTIVE;
    }
    ...
    packetHeader->header_bytes = NXPC_DBG_PACKET_HEADER_BYTES;
    packetHeader->flags =
        (s_streamDroppedBeforePacket != 0U) ? NXPC_DBG_PACKET_FLAG_DROPPED_BEFORE : 0U;
    packetHeader->msg_id = NXPC_DBG_STATS_REPORT;
    packetHeader->sequence = s_streamSequence;
    packetHeader->payload_length = NXPC_DBG_STATS_REPORT_BYTES;
    packetHeader->arg0 = s_streamDroppedFrameCount;
    packetHeader->arg1 = s_streamBusyCount;
    packetHeader->arg2 = s_streamSendErrorCount;

    stats = (nxpc_dbg_stats_report_t *)&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES];
...
static void nxpc_usb_debug_stream__start_source(nxpc_usb_debug_stream_source_t source)
{
    ...
    s_streamStatsDue =
        ((s_statsEnabled != 0U) && (source == NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA)) ? 1U : 0U;
    s_streamDroppedBeforePacket = 0U;
...
void nxpc_usb_debug_stream__start(void)
{
    s_streamSequence = 0U;
    s_statsEnabled = 1U;
    nxpc_usb_debug_stream__start_source(NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA);
}

static void nxpc_usb_debug_stream__start_synthetic(void)
{
    s_streamSequence = 0U;
    s_statsEnabled = 0U;
    nxpc_usb_debug_stream__start_source(NXPC_USB_DEBUG_STREAM_SOURCE_SYNTHETIC);
}
...
bool nxpc_usb_debug_stream__camera_frames_active(void)
{
    return (s_sessionActive != 0U) && (s_streamEnabled != 0U) &&
           ...
           nxpc_usb_debug_stream__is_open();
}
...
static uint32_t nxpc_usb_debug_stream__bounded_text_length(const char *text, uint32_t maxLength)
{
    uint32_t length = 0U;
...
static bool nxpc_usb_debug_stream__framework_telemetry_name(const char *name)
{
    static const char *const protectedNames[] = {
        "motor.enabled", "motor.left.command", "motor.right.command", "steering.command",
        "system.mode",   "system.state",       "system.test_page",
    };
...
bool nxpc_usb_debug_stream__log_text(uint8_t level, const char *category, const char *text)
{
    ...
    nxpc_usb_log_record_t *queuedRecord;
    uint32_t usbOsaCurrentSr;
    uint8_t queued = 0U;
    uint8_t tail;

    if ((s_logEnabled == 0U) || (s_sessionActive == 0U) || !nxpc_usb_debug_stream__is_open() ||
        (level > NXPC_DBG_LOG_LEVEL_ERROR) || (category == NULL) || (text == NULL))
    {
        return false;
    }
    ...
    record.recordId = 0U;
    record.level = level;
    record.categoryLength = (uint8_t)nxpc_usb_debug_stream__bounded_text_length(
        category, NXPC_USB_LOG_CATEGORY_MAX_BYTES);
    record.textLength =
        (uint16_t)nxpc_usb_debug_stream__bounded_text_length(text, NXPC_USB_LOG_TEXT_MAX_BYTES);
    (void)memcpy(record.category, category, record.categoryLength);
    (void)memcpy(record.text, text, record.textLength);

    usbOsaCurrentSr = DisableGlobalIRQ();
    if ((s_logEnabled != 0U) && (s_logCount < NXPC_USB_LOG_QUEUE_DEPTH))
    {
        tail = (uint8_t)(s_logHead + s_logCount);
        if (tail >= NXPC_USB_LOG_QUEUE_DEPTH)
        {
            tail = (uint8_t)(tail - NXPC_USB_LOG_QUEUE_DEPTH);
        }
...
{
    char text[NXPC_USB_LOG_TEXT_MAX_BYTES + 1U];
    va_list arguments;
    int result;
    ...
    return nxpc_usb_debug_stream__log_text(level, category, text);
}

static bool nxpc_usb_debug_stream__telemetry_scalar(const char *name, uint8_t valueType,
                                                    uint32_t valueBits, const char *units,
                                                    const char *text, bool frameworkValue)
{
    ...
    if ((s_telemetryEnabled == 0U) || (s_sessionActive == 0U) ||
        !nxpc_usb_debug_stream__is_open() || (name == NULL) || (name[0] == '\0') ||
        (units == NULL) || (valueType < NXPC_DBG_TELEMETRY_TYPE_I32) ||
        (valueType > NXPC_DBG_TELEMETRY_TYPE_TEXT))
    {
        return false;
    }
    ...
    if (valueType == NXPC_DBG_TELEMETRY_TYPE_TEXT)
    {
        ...
        textLength = nxpc_usb_debug_stream__bounded_text_length(
            text, NXPC_DBG_TELEMETRY_TEXT_MAX_BYTES + 1U);
    ...
    record.nameLength = (uint16_t)nxpc_usb_debug_stream__bounded_text_length(
        name, NXPC_USB_TELEMETRY_NAME_MAX_BYTES);
    record.unitsLength = (uint8_t)nxpc_usb_debug_stream__bounded_text_length(
        units, NXPC_USB_TELEMETRY_UNITS_MAX_BYTES);
...
{
    return nxpc_usb_debug_stream__telemetry_scalar(name, NXPC_DBG_TELEMETRY_TYPE_I32,
                                                   (uint32_t)value, units, NULL, false);
}
...
bool nxpc_usb_debug_stream__telemetry_f32(const char *name, float value, const char *units)
{
    ...
    return nxpc_usb_debug_stream__telemetry_scalar(name, NXPC_DBG_TELEMETRY_TYPE_F32, valueBits,
                                                   units, NULL, false);
}

bool nxpc_usb_debug_stream__telemetry_bool(const char *name, bool value)
{
    return nxpc_usb_debug_stream__telemetry_scalar(name, NXPC_DBG_TELEMETRY_TYPE_BOOL,
                                                   value ? 1U : 0U, "", NULL, false);
}

bool nxpc_usb_debug_stream__telemetry_text(const char *name, const char *value)
{
    return nxpc_usb_debug_stream__telemetry_scalar(name, NXPC_DBG_TELEMETRY_TYPE_TEXT, 0U, "",
                                                   value, false);
}
...
bool nxpc_usb_debug_stream__publish_frame(const uint16_t *frame)
{
    ...
    if ((frame == NULL) || (s_streamEnabled == 0U) || !nxpc_usb_debug_stream__is_open())
    {
        return false;
    }

    usbOsaCurrentSr = DisableGlobalIRQ();
    if ((s_streamEnabled != 0U) && (s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA) &&
        (s_streamFrameActive == 0U))
    {
    ...
    else if ((s_streamEnabled != 0U) && (s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA))
    {
...
static uint8_t nxpc_usb_debug_stream__queue_control_response(uint32_t msgId,
                                                             uint32_t requestSequence,
                                                             uint32_t status, const void *payload,
                                                             uint32_t payloadLength)
{
    nxpc_usb_control_response_t *response;
    uint8_t queued = 0U;
    uint8_t tail;
    uint32_t usbOsaCurrentSr;

    if (payloadLength > NXPC_USB_CONTROL_RESPONSE_PAYLOAD_BYTES)
    {
        return 0U;
    }

    usbOsaCurrentSr = DisableGlobalIRQ();
    if (s_controlResponseCount < NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH)
    {
        tail = (uint8_t)(s_controlResponseHead + s_controlResponseCount);
        if (tail >= NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH)
        {
            tail = (uint8_t)(tail - NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH);
        }
...
{
    (void)nxpc_usb_debug_stream__queue_control_response(NXPC_DBG_CONTROL_SYSTEM_ACTION,
                                                        requestSequence, status, NULL, 0U);
    nxpc_usb_debug_stream__schedule_tx();
}
...
static uint8_t nxpc_usb_debug_stream__send_control_response(void)
{
    ...
    packetHeader->version = NXPC_DBG_VERSION;
    packetHeader->header_bytes = NXPC_DBG_PACKET_HEADER_BYTES;
    packetHeader->flags = NXPC_DBG_PACKET_FLAG_RESPONSE;
    ...
    {
        (void)memcpy(&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES], response.payload,
                     response.payloadLength);
    }

    packetBytes = NXPC_DBG_PACKET_HEADER_BYTES + response.payloadLength;
    if (nxpc_usb_debug_stream__submit_packet(packetBytes) != 0U)
    {
        usbOsaCurrentSr = DisableGlobalIRQ();
        if (s_controlResponseCount != 0U)
        {
            ...
            if (s_controlResponseHead >= NXPC_USB_CONTROL_RESPONSE_QUEUE_DEPTH)
            {
        ...
        if (s_txControlBurst < NXPC_USB_TX_CONTROL_BURST_MAX)
        {
            s_txControlBurst++;
        }
        return 1U;
...
static uint8_t nxpc_usb_debug_stream__send_log_record(void)
{
    nxpc_usb_log_record_t record;
    nxpc_dbg_log_record_t *wireRecord;
    nxpc_dbg_packet_header_t *packetHeader;
    uint32_t packetBytes;
    ...
    payloadBytes = NXPC_DBG_LOG_RECORD_HEADER_BYTES + record.categoryLength + record.textLength;
    packetHeader = (nxpc_dbg_packet_header_t *)s_streamTxBuf;
    packetHeader->magic = NXPC_DBG_MAGIC;
    packetHeader->version = NXPC_DBG_VERSION;
    packetHeader->header_bytes = NXPC_DBG_PACKET_HEADER_BYTES;
    packetHeader->flags = 0U;
    packetHeader->msg_id = NXPC_DBG_LOG_TEXT;
    packetHeader->sequence = s_streamSequence;
    ...
    wireRecord = (nxpc_dbg_log_record_t *)&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES];
    ...
    (void)memcpy(&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_LOG_RECORD_HEADER_BYTES],
                 record.category, record.categoryLength);
    (void)memcpy(&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_LOG_RECORD_HEADER_BYTES +
                                record.categoryLength],
                 record.text, record.textLength);
    ...
    {
        s_logHead++;
        if (s_logHead >= NXPC_USB_LOG_QUEUE_DEPTH)
        {
            s_logHead = 0U;
        }
        s_logCount--;
    ...
    if (s_txDiagnosticBurst < NXPC_USB_TX_DIAGNOSTIC_BURST_MAX)
    {
        s_txDiagnosticBurst++;
    }
    return 1U;
...
static uint8_t nxpc_usb_debug_stream__send_telemetry_scalar(void)
{
    nxpc_usb_telemetry_scalar_t record;
    nxpc_dbg_telemetry_scalar_t *wireRecord;
    nxpc_dbg_packet_header_t *packetHeader;
    uint32_t packetBytes;
    uint32_t payloadBytes;
    uint32_t usbOsaCurrentSr;
    ...
    packetHeader->header_bytes = NXPC_DBG_PACKET_HEADER_BYTES;
    packetHeader->flags = 0U;
    packetHeader->msg_id = NXPC_DBG_TELEMETRY_SCALAR;
    packetHeader->sequence = s_streamSequence;
    packetHeader->payload_length = payloadBytes;
    ...
    wireRecord = (nxpc_dbg_telemetry_scalar_t *)&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES];
    wireRecord->timestamp_ms = record.timestampMs;
    ...
    (void)memcpy(
        &s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_TELEMETRY_SCALAR_HEADER_BYTES],
        record.name, record.nameLength);
    (void)memcpy(&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES +
                                NXPC_DBG_TELEMETRY_SCALAR_HEADER_BYTES + record.nameLength],
                 record.units, record.unitsLength);
    (void)memcpy(
        &s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES + NXPC_DBG_TELEMETRY_SCALAR_HEADER_BYTES +
    ...
    {
        s_telemetryHead++;
        if (s_telemetryHead >= NXPC_USB_TELEMETRY_QUEUE_DEPTH)
        {
            s_telemetryHead = 0U;
        }
        s_telemetryCount--;
...
static void nxpc_usb_debug_stream__handle_control_packet(const nxpc_dbg_packet_header_t *request)
{
    ...
    if ((request->flags & NXPC_DBG_PACKET_FLAG_RESPONSE) != 0U)
    {
        return;
    }
    ...
    case NXPC_DBG_CONTROL_HELLO:
    {
        ...
        hello.max_packet_bytes = NXPC_USB_STREAM_TX_BYTES;
        hello.frame_width = (uint16_t)NXPC_USB_STREAM_FRAME_WIDTH;
        hello.frame_height = (uint16_t)NXPC_USB_STREAM_FRAME_HEIGHT;
        hello.pixel_format = NXPC_DBG_PIXEL_FORMAT_RGB565_LE;
        hello.reserved = 0U;
        hello.session_id = s_sessionId;

        (void)nxpc_usb_debug_stream__queue_control_response(
            NXPC_DBG_CONTROL_HELLO, request->sequence, NXPC_DBG_CONTROL_STATUS_OK, &hello,
            sizeof(hello));
        break;
    }

    case NXPC_DBG_CONTROL_SET_CHANNELS:
        if (s_sessionActive == 0U)
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_SET_CHANNELS, request->sequence,
                NXPC_DBG_CONTROL_STATUS_SESSION_REQUIRED, NULL, 0U);
            break;
        }

        requestedChannels = request->arg0;
        requestedSource = request->arg1;
        if (((requestedChannels & ~NXPC_USB_CONTROL_SUPPORTED_CHANNELS) != 0U) ||
            (((requestedChannels & NXPC_DBG_CHANNEL_STATS) != 0U) &&
             ((requestedChannels & NXPC_DBG_CHANNEL_FRAMES) == 0U)) ||
            (((requestedChannels & NXPC_DBG_CHANNEL_STATS) != 0U) &&
             (requestedSource == NXPC_DBG_STREAM_SOURCE_SYNTHETIC)) ||
            (requestedSource > NXPC_DBG_STREAM_SOURCE_SYNTHETIC))
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_SET_CHANNELS, request->sequence,
                NXPC_DBG_CONTROL_STATUS_BAD_ARGUMENT, NULL, 0U);
            break;
        }
        ...
        if ((requestedChannels & NXPC_DBG_CHANNEL_FRAMES) == 0U)
        {
            nxpc_usb_debug_stream__stop();
        }
        else
        {
            s_statsEnabled = ((requestedChannels & NXPC_DBG_CHANNEL_STATS) != 0U) ? 1U : 0U;
            nxpc_usb_debug_stream__start_source((nxpc_usb_debug_stream_source_t)requestedSource);
        }

        (void)nxpc_usb_debug_stream__queue_control_response(
            NXPC_DBG_CONTROL_SET_CHANNELS, request->sequence, NXPC_DBG_CONTROL_STATUS_OK, NULL, 0U);
        if ((s_logEnabled != 0U) && (logWasEnabled == 0U))
        {
            (void)nxpc_usb_debug_stream__log_text(NXPC_DBG_LOG_LEVEL_INFO, "system",
    ...
    case NXPC_DBG_CONTROL_PING:
        (void)nxpc_usb_debug_stream__queue_control_response(
            NXPC_DBG_CONTROL_PING, request->sequence,
            (s_sessionActive != 0U) ? NXPC_DBG_CONTROL_STATUS_OK
                                    : NXPC_DBG_CONTROL_STATUS_SESSION_REQUIRED,
            NULL, 0U);
        break;

    case NXPC_DBG_CONTROL_CLOSE:
        nxpc_usb_debug_stream__stop();
        (void)nxpc_usb_debug_stream__queue_control_response(
            NXPC_DBG_CONTROL_CLOSE, request->sequence, NXPC_DBG_CONTROL_STATUS_OK, NULL, 0U);
        s_logEnabled = 0U;
        s_logHead = 0U;
        s_logCount = 0U;
        s_telemetryEnabled = 0U;
    ...
    case NXPC_DBG_CONTROL_ENTER_ISP:
        ...
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_ENTER_ISP, request->sequence,
                NXPC_DBG_CONTROL_STATUS_SESSION_REQUIRED, NULL, 0U);
            break;
        }
        if ((request->arg0 != NXPC_DBG_ENTER_ISP_CONFIRMATION) || (request->arg1 != 0U) ||
            (request->arg2 != 0U))
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_ENTER_ISP, request->sequence, NXPC_DBG_CONTROL_STATUS_BAD_ARGUMENT,
                NULL, 0U);
            break;
        }
        ...
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_ENTER_ISP, request->sequence, NXPC_DBG_CONTROL_STATUS_BUSY, NULL,
                0U);
            break;
        }
        if (nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_ENTER_ISP, request->sequence, NXPC_DBG_CONTROL_STATUS_OK, NULL,
                0U) != 0U)
        {
            nxpc_usb_debug_stream__stop();
            s_logEnabled = 0U;
    ...
    case NXPC_DBG_CONTROL_SYSTEM_ACTION:
        ...
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_SYSTEM_ACTION, request->sequence,
                NXPC_DBG_CONTROL_STATUS_SESSION_REQUIRED, NULL, 0U);
            break;
        }
        ...
        {
            (void)nxpc_usb_debug_stream__queue_control_response(
                NXPC_DBG_CONTROL_SYSTEM_ACTION, request->sequence, NXPC_DBG_CONTROL_STATUS_BUSY,
                NULL, 0U);
            break;
        }
    ...
    default:
        (void)nxpc_usb_debug_stream__queue_control_response(
            NXPC_DBG_CONTROL_ERROR, request->sequence, NXPC_DBG_CONTROL_STATUS_UNSUPPORTED, NULL,
            0U);
        break;
    }
}

static void nxpc_usb_debug_stream__consume_control_rx(uint32_t bytes)
{
...
static void nxpc_usb_debug_stream__handle_received_data(const uint8_t *data, uint32_t length)
{
    nxpc_dbg_packet_header_t request;
    uint32_t packetBytes;
    ...
    if (length > (NXPC_USB_CONTROL_RX_BYTES - s_controlRxLength))
    {
        s_controlRxLength = 0U;
        return;
    }
    ...
    while (s_controlRxLength >= NXPC_DBG_PACKET_HEADER_BYTES)
    {
        (void)memcpy(&request, s_controlRxBuffer, sizeof(request));
        if (request.magic != NXPC_DBG_MAGIC)
        {
            nxpc_usb_debug_stream__consume_control_rx(1U);
            continue;
        }

        if ((request.version != NXPC_DBG_VERSION) ||
            (request.header_bytes != NXPC_DBG_PACKET_HEADER_BYTES) ||
            (request.payload_length > NXPC_USB_CONTROL_MAX_PAYLOAD_BYTES))
        {
            nxpc_usb_debug_stream__consume_control_rx(1U);
            continue;
        }

        packetBytes = NXPC_DBG_PACKET_HEADER_BYTES + request.payload_length;
        ...
        nxpc_usb_debug_stream__handle_control_packet(&request);
        nxpc_usb_debug_stream__consume_control_rx(packetBytes);
    }
}

static uint8_t nxpc_usb_debug_stream__send_next_diagnostic(void)
{
    if ((s_txPreferTelemetry != 0U) && (s_telemetryCount != 0U) &&
        (nxpc_usb_debug_stream__send_telemetry_scalar() != 0U))
    {
        s_txPreferTelemetry = 0U;
        return 1U;
    }

    if ((s_logCount != 0U) && (nxpc_usb_debug_stream__send_log_record() != 0U))
    {
        s_txPreferTelemetry = 1U;
        return 1U;
    }

    if ((s_telemetryCount != 0U) && (nxpc_usb_debug_stream__send_telemetry_scalar() != 0U))
    {
        s_txPreferTelemetry = 0U;
        return 1U;
    }
...
static void nxpc_usb_debug_stream__schedule_tx(void)
{
    ...
    nxpc_dbg_packet_header_t *packetHeader;
    nxpc_dbg_rui_write_frame_buffer_raw_t *frameChunk;
    uint8_t framePending;
    uint8_t nonControlPending;

    if (!nxpc_usb_debug_stream__is_open())
    {
        return;
    }
    ...
    framePending =
        ((s_streamEnabled != 0U) && ((s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_SYNTHETIC) ||
                                     (s_streamFrameActive != 0U)))
            ? 1U
            : 0U;
    nonControlPending = ((s_streamStatsDue != 0U) || (s_logCount != 0U) ||
                         (s_telemetryCount != 0U) || (framePending != 0U))
                            ? 1U
                            : 0U;
    ...
    if ((s_controlResponseCount != 0U) &&
        ((s_txControlBurst < NXPC_USB_TX_CONTROL_BURST_MAX) || (nonControlPending == 0U)) &&
        (nxpc_usb_debug_stream__send_control_response() != 0U))
    {
        return;
    }

    if (nxpc_usb_debug_stream__send_stats_report() != 0U)
    {
        s_txControlBurst = 0U;
        if (s_txDiagnosticBurst < NXPC_USB_TX_DIAGNOSTIC_BURST_MAX)
        {
    ...
    if (((s_logCount != 0U) || (s_telemetryCount != 0U)) &&
        ((s_txDiagnosticBurst < NXPC_USB_TX_DIAGNOSTIC_BURST_MAX) || (framePending == 0U)) &&
        (nxpc_usb_debug_stream__send_next_diagnostic() != 0U))
    {
        s_txControlBurst = 0U;
        return;
    }
    ...
    if (s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA)
    {
        ...
        if ((s_cameraFrameGeneration - s_streamFrameGeneration) >=
            CONFIG__CAMERA_FRAME_BUFFER_COUNT)
        {
            s_streamFrameData = NULL;
            s_streamFrameActive = 0U;
            s_streamOffset = 0U;
    ...
    remaining = NXPC_USB_STREAM_FRAME_BYTES - s_streamOffset;
    payloadBytes = (remaining > NXPC_USB_STREAM_PAYLOAD_MAX_BYTES)
                       ? NXPC_USB_STREAM_PAYLOAD_MAX_BYTES
                       : remaining;
    if (s_streamOffset == 0U)
    {
        chunkFlags |= NXPC_DBG_RUI_CHUNK_FRAME_START;
    }
    if (payloadBytes == remaining)
    {
        chunkFlags |= NXPC_DBG_RUI_CHUNK_FRAME_END;
    }

    if (sourceFrameData != NULL)
    {
        (void)memcpy(&s_streamTxBuf[NXPC_USB_STREAM_DATA_OFFSET], &sourceFrameData[s_streamOffset],
                     payloadBytes);
        if ((s_cameraFrameGeneration - s_streamFrameGeneration) >=
            CONFIG__CAMERA_FRAME_BUFFER_COUNT)
        {
    ...
    packetPayloadBytes = NXPC_DBG_RUI_WRITE_FRAME_BUFFER_RAW_HEADER_BYTES + payloadBytes;
    ...
    packetHeader->header_bytes = NXPC_DBG_PACKET_HEADER_BYTES;
    packetHeader->flags =
        ((s_streamDroppedBeforePacket != 0U) || (s_streamFrameResyncPending != 0U))
            ? NXPC_DBG_PACKET_FLAG_DROPPED_BEFORE
            : 0U;
    packetHeader->msg_id = NXPC_DBG_RUI_WRITE_FRAME_BUFFER_RAW;
    packetHeader->sequence = s_streamSequence;
    ...
    frameChunk =
        (nxpc_dbg_rui_write_frame_buffer_raw_t *)&s_streamTxBuf[NXPC_DBG_PACKET_HEADER_BYTES];
    frameChunk->frame_id = s_streamFrameId;
    frameChunk->byte_offset = s_streamOffset;
    frameChunk->total_frame_bytes = NXPC_USB_STREAM_FRAME_BYTES;
    frameChunk->width = (uint16_t)NXPC_USB_STREAM_FRAME_WIDTH;
    frameChunk->height = (uint16_t)NXPC_USB_STREAM_FRAME_HEIGHT;
    frameChunk->pixel_format = NXPC_DBG_PIXEL_FORMAT_RGB565_LE;
    frameChunk->buffer_id = 0U;
    frameChunk->chunk_flags = chunkFlags;

    if (nxpc_usb_debug_stream__submit_packet(NXPC_DBG_PACKET_HEADER_BYTES + packetPayloadBytes) !=
        0U)
    {
        s_txControlBurst = 0U;
        s_txDiagnosticBurst = 0U;
        s_streamDroppedBeforePacket = 0U;
        if ((chunkFlags & NXPC_DBG_RUI_CHUNK_FRAME_START) != 0U)
        {
            s_streamFrameResyncPending = 0U;
        }
        if ((chunkFlags & NXPC_DBG_RUI_CHUNK_FRAME_END) != 0U)
        {
            s_streamOffset = 0U;
            if (s_streamSource == NXPC_USB_DEBUG_STREAM_SOURCE_CAMERA)
            {
            ...
            nxpc_usb_debug_stream__mark_frame_completed();
        }
        else
        {
...
static void nxpc_usb_debug_stream__clock_init(void)
{
    ...
    USB_EhciPhyInit(NXPC_USB_CONTROLLER_ID, BOARD_XTAL0_CLK_HZ, &phyConfig);
}

static void nxpc_usb_debug_stream__isr_enable(void)
{
    uint8_t irqNumber;
    uint8_t usbDeviceEhciIrq[] = USBHS_IRQS;

    irqNumber = usbDeviceEhciIrq[NXPC_USB_CONTROLLER_ID - kUSB_ControllerEhci0];
    NVIC_SetPriority((IRQn_Type)irqNumber, NXPC_USB_INTERRUPT_PRIORITY);
...
void nxpc_usb_debug_stream__init(void)
{
    ...
    nxpc_usb_debug_stream__clock_init();
    ...
    s_profileMaxCycles = 0U;
#endif
    nxpc_usb_debug_stream__fill_static_payload();

    if (USB_DeviceClassInit(NXPC_USB_CONTROLLER_ID, &s_cdcAcmConfigList, &s_cdcVcom.deviceHandle) !=
        kStatus_USB_Success)
    {
        (void)DbgConsole_Printf("nxpc_usb: USB_DeviceClassInit failed\r\n");
        return;
    }

    s_cdcVcom.cdcAcmHandle = s_cdcAcmConfigList.config->classHandle;
    nxpc_usb_debug_stream__isr_enable();

    SDK_DelayAtLeastUs(5000U, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    USB_DeviceRun(s_cdcVcom.deviceHandle);
    s_usbInitialized = 1U;

    (void)DbgConsole_Printf("nxpc_usb: CDC debug stream initialized\r\n");
}
...
void nxpc_usb_debug_stream__service(void)
{
    ...
    {
#if CONFIG__USB_DEBUG_PROFILE_ENABLE
        nxpc_usb_debug_stream__profile_service(profileStartCycles, 0U);
#endif
        return;
    }
    ...
    if ((s_recvSize != 0U) && (s_recvSize != USB_CANCELLED_TRANSFER_LENGTH))
    {
        ...
        nxpc_usb_debug_stream__handle_received_data(s_currRecvBuf, recvSize);

        error =
            USB_DeviceCdcAcmRecv(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_OUT_ENDPOINT,
                                 s_currRecvBuf, g_UsbDeviceCdcVcomDicEndpoints[1].maxPacketSize);
        if (error != kStatus_USB_Success)
        {
            (void)error;
        }
    ...
    nxpc_usb_debug_stream__schedule_tx();
#if CONFIG__USB_DEBUG_PROFILE_ENABLE
    nxpc_usb_debug_stream__profile_service(profileStartCycles, 1U);
#endif
}
                                                        ...
                                                        void *param)
{
    ...
    nxpc_usb_cdc_acm_info_t *acmInfo = &s_usbCdcAcmInfo;
    ...
    case kUSB_DeviceCdcEventGetCommFeature:
        ...
        {
            *(acmReqParam->buffer) = s_abstractState;
            *(acmReqParam->length) = NXPC_USB_COMM_FEATURE_DATA_SIZE;
            error = kStatus_USB_Success;
        }
        else if (acmReqParam->setupValue == USB_DEVICE_CDC_FEATURE_COUNTRY_SETTING)
        {
    ...
    case kUSB_DeviceCdcEventGetLineCoding:
        ...
        *(acmReqParam->length) = NXPC_USB_LINE_CODING_SIZE;
        error = kStatus_USB_Success;
        break;
    ...
    case kUSB_DeviceCdcEventSetControlLineState:
        ...
        acmInfo->serialStateBuf[0] = NXPC_USB_NOTIF_REQUEST_TYPE;
        ...
        acmInfo->serialStateBuf[6] = NXPC_USB_UART_BITMAP_SIZE;
        acmInfo->serialStateBuf[7] = 0x00U;
        uartBitmap =
            (uint8_t *)&acmInfo
                ->serialStateBuf[NXPC_USB_NOTIF_PACKET_SIZE + NXPC_USB_UART_BITMAP_SIZE - 2U];
        uartBitmap[0] = (uint8_t)(acmInfo->uartState & 0xFFU);
        uartBitmap[1] = (uint8_t)((acmInfo->uartState >> 8U) & 0xFFU);

#if ((defined USB_DEVICE_CONFIG_CDC_CIC_EP_DISABLE) && (USB_DEVICE_CONFIG_CDC_CIC_EP_DISABLE > 0U))
#else
        len = (uint32_t)(NXPC_USB_NOTIF_PACKET_SIZE + NXPC_USB_UART_BITMAP_SIZE);
                                                           ...
                                                           void *param)
{
    ...
    case kUSB_DeviceEventBusReset:
        ...
        if (USB_DeviceClassGetSpeed(NXPC_USB_CONTROLLER_ID, &s_cdcVcom.speed) ==
            kStatus_USB_Success)
        {
    ...
    case kUSB_DeviceEventSetConfiguration:
        ...
        else if (*temp8 == USB_CDC_VCOM_CONFIGURE_INDEX)
        {
            ...
            error = kStatus_USB_Success;
            (void)USB_DeviceCdcAcmRecv(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_OUT_ENDPOINT,
                                       s_currRecvBuf,
                                       g_UsbDeviceCdcVcomDicEndpoints[1].maxPacketSize);
        }
        break;
    ...
    case kUSB_DeviceEventGetInterface:
        if (param != NULL)
        {
            ...
            {
                *temp16 =
                    (*temp16 & 0xFF00U) | s_cdcVcom.currentInterfaceAlternateSetting[interface];
                error = kStatus_USB_Success;
            }
        }
        break;
