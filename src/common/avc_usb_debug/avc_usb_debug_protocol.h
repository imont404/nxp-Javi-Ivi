...
#define AVC_DBG_LOG_RECORD_HEADER_BYTES (12u)
#define AVC_DBG_TELEMETRY_SCALAR_HEADER_BYTES (16u)
#define AVC_DBG_STATS_REPORT_BYTES (76u)
#define AVC_DBG_CONTROL_HELLO_RESPONSE_BYTES (24u)
...
#define AVC_DBG_ENTER_ISP_CONFIRMATION (0x21505349u)

#define AVC_DBG_CAPABILITY_FRAMED_CONTROL (1u << 0)
#define AVC_DBG_CAPABILITY_CAMERA_FRAMES (1u << 1)
#define AVC_DBG_CAPABILITY_SYNTHETIC_FRAMES (1u << 2)
#define AVC_DBG_CAPABILITY_STREAM_STATS (1u << 3)
#define AVC_DBG_CAPABILITY_LOG_TEXT (1u << 4)
#define AVC_DBG_CAPABILITY_NAMED_TELEMETRY (1u << 5)
#define AVC_DBG_CAPABILITY_ENTER_ISP (1u << 6)

#define AVC_DBG_CHANNEL_FRAMES (1u << 0)
#define AVC_DBG_CHANNEL_STATS (1u << 1)
#define AVC_DBG_CHANNEL_LOGS (1u << 2)
#define AVC_DBG_CHANNEL_TELEMETRY (1u << 3)

#define AVC_DBG_CONTROL_STATUS_OK (0u)
#define AVC_DBG_CONTROL_STATUS_BAD_PACKET (1u)
#define AVC_DBG_CONTROL_STATUS_UNSUPPORTED (2u)
#define AVC_DBG_CONTROL_STATUS_SESSION_REQUIRED (3u)
#define AVC_DBG_CONTROL_STATUS_BUSY (4u)
#define AVC_DBG_CONTROL_STATUS_BAD_ARGUMENT (5u)
...
#define AVC_DBG_LOG_LEVEL_TRACE (0u)
#define AVC_DBG_LOG_LEVEL_DEBUG (1u)
#define AVC_DBG_LOG_LEVEL_INFO (2u)
#define AVC_DBG_LOG_LEVEL_WARNING (3u)
#define AVC_DBG_LOG_LEVEL_ERROR (4u)

#define AVC_DBG_TELEMETRY_TYPE_I32 (1u)
#define AVC_DBG_TELEMETRY_TYPE_U32 (2u)
#define AVC_DBG_TELEMETRY_TYPE_F32 (3u)
#define AVC_DBG_TELEMETRY_TYPE_BOOL (4u)
...
typedef struct AVC_DBG_PACKED avc_dbg_log_record
{
...
typedef struct AVC_DBG_PACKED avc_dbg_telemetry_scalar
{
    uint32_t timestamp_ms;
    uint32_t sample_id;
    uint32_t value_bits;
...
typedef struct AVC_DBG_PACKED avc_dbg_control_hello_response
{
    ...
    uint16_t reserved;
    uint32_t session_id;
} avc_dbg_control_hello_response_t;
...
static_assert(sizeof(avc_dbg_stats_report_t) == AVC_DBG_STATS_REPORT_BYTES,
              "AVC debug stats report wire size changed unexpectedly");
static_assert(sizeof(avc_dbg_log_record_t) == AVC_DBG_LOG_RECORD_HEADER_BYTES,
              "AVC debug log record header wire size changed unexpectedly");
static_assert(sizeof(avc_dbg_telemetry_scalar_t) == AVC_DBG_TELEMETRY_SCALAR_HEADER_BYTES,
              "AVC debug telemetry scalar header wire size changed unexpectedly");
static_assert(sizeof(avc_dbg_control_hello_response_t) ==
                  AVC_DBG_CONTROL_HELLO_RESPONSE_BYTES,
              "AVC debug hello response must stay 24 bytes");
...
_Static_assert(sizeof(avc_dbg_stats_report_t) == AVC_DBG_STATS_REPORT_BYTES,
               "AVC debug stats report wire size changed unexpectedly");
_Static_assert(sizeof(avc_dbg_log_record_t) == AVC_DBG_LOG_RECORD_HEADER_BYTES,
               "AVC debug log record header wire size changed unexpectedly");
_Static_assert(sizeof(avc_dbg_telemetry_scalar_t) == AVC_DBG_TELEMETRY_SCALAR_HEADER_BYTES,
               "AVC debug telemetry scalar header wire size changed unexpectedly");
_Static_assert(sizeof(avc_dbg_control_hello_response_t) ==
                   AVC_DBG_CONTROL_HELLO_RESPONSE_BYTES,
               "AVC debug hello response must stay 24 bytes");
#endif
