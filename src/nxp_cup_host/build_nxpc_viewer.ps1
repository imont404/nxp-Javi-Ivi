...
$buildDir = Join-Path $repoRoot "build/host/nxp_cup_host"
...
Write-Host "Configuring NXP Cup native viewer..."
& cmake @configureArgs
if ($LASTEXITCODE -ne 0) {
    throw "CMake configure failed with exit code $LASTEXITCODE"
}

Write-Host "Building NXP Cup native viewer ($Configuration)..."
& cmake --build $buildDir --config $Configuration --target nxpc_viewer nxpc_tool
if ($LASTEXITCODE -ne 0) {
