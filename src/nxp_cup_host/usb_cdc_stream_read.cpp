...
constexpr uint32_t kHeaderSize = NXPC_DBG_PACKET_HEADER_BYTES;
constexpr uint32_t kRuiFrameChunkHeaderSize = NXPC_DBG_RUI_WRITE_FRAME_BUFFER_RAW_HEADER_BYTES;
constexpr uint32_t kStatsReportSize = NXPC_DBG_STATS_REPORT_BYTES;
...
constexpr uint32_t kKnownPacketFlags = NXPC_DBG_PACKET_FLAG_RESPONSE | NXPC_DBG_PACKET_FLAG_MORE |
                                       NXPC_DBG_PACKET_FLAG_PAYLOAD_CRC32 | NXPC_DBG_PACKET_FLAG_DROPPED_BEFORE;
constexpr uint32_t kKnownChunkFlags = NXPC_DBG_RUI_CHUNK_FRAME_START | NXPC_DBG_RUI_CHUNK_FRAME_END |
...
struct Parser
{
    ...
    void parse()
    {
        while ((buffer.size() - cursor) >= kHeaderSize)
        {
            ...
            bool isFramePacket = msgId == NXPC_DBG_RUI_WRITE_FRAME_BUFFER_RAW;
            bool isStatsPacket = msgId == NXPC_DBG_STATS_REPORT;
            bool isControlPacket =
                ((packetFlags & NXPC_DBG_PACKET_FLAG_RESPONSE) != 0U) &&
                ((msgId == NXPC_DBG_CONTROL_HELLO) || (msgId == NXPC_DBG_CONTROL_SET_CHANNELS) ||
                 (msgId == NXPC_DBG_CONTROL_PING) || (msgId == NXPC_DBG_CONTROL_CLOSE) ||
                 (msgId == NXPC_DBG_CONTROL_ERROR));
            ...
            bool validHeader = (std::memcmp(header, kMagic, sizeof(kMagic)) == 0) && (version == 1U) &&
                               ...
                               ((isFramePacket && (payloadLen > kRuiFrameChunkHeaderSize)) ||
                                ...
                                (isControlPacket && (payloadLen <= NXPC_DBG_CONTROL_HELLO_RESPONSE_BYTES) &&
                                 ((msgId != NXPC_DBG_CONTROL_HELLO) ||
                                  (payloadLen == NXPC_DBG_CONTROL_HELLO_RESPONSE_BYTES))));
            if (!validHeader)
            {
                cursor++;
                malformedHeaders++;
            ...
            if ((packetFlags & NXPC_DBG_PACKET_FLAG_DROPPED_BEFORE) != 0U)
            {
            ...
            bool validChunk = (frameBytes == kFrameBytes) && (width == 320U) && (height == 200U) &&
                              (pixelFormat == NXPC_DBG_PIXEL_FORMAT_RGB565_LE) && (bufferId == 0U) &&
            ...
            if (((packetFlags & NXPC_DBG_PACKET_FLAG_DROPPED_BEFORE) != 0U) &&
                ((chunkFlags & NXPC_DBG_RUI_CHUNK_FRAME_START) != 0U))
            {
            ...
            if ((offset == 0U) && ((chunkFlags & NXPC_DBG_RUI_CHUNK_FRAME_START) == 0U))
            {
                sequenceErrors++;
    ...
    nxpc_dbg_stats_report_t lastStats{};
...
nxpc_dbg_packet_header_t buildControlPacket(uint32_t requestSequence,
                                           uint32_t msgId,
                                           uint32_t arg0 = 0U,
                                           uint32_t arg1 = 0U,
                                           uint32_t arg2 = 0U)
...
void sendControl(SerialPort &port,
                 ...
                 uint32_t arg2 = 0U)
{
    nxpc_dbg_packet_header_t packet = buildControlPacket(requestSequence, msgId, arg0, arg1, arg2);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        Sleep(250);
        port.clearInput();
        sendControl(port, 0U, NXPC_DBG_CONTROL_HELLO);
        ControlResponse hello = waitForControlResponse(port, parser, NXPC_DBG_CONTROL_HELLO, 0U);
        if (hello.status != NXPC_DBG_CONTROL_STATUS_OK)
        {
        ...
        uint32_t streamSource = synthetic ? NXPC_DBG_STREAM_SOURCE_SYNTHETIC : NXPC_DBG_STREAM_SOURCE_CAMERA;
        uint32_t channels = NXPC_DBG_CHANNEL_FRAMES | (synthetic ? 0U : NXPC_DBG_CHANNEL_STATS);
        sendControl(port, 1U, NXPC_DBG_CONTROL_SET_CHANNELS, channels, streamSource);
        ControlResponse startResponse =
            waitForControlResponse(port, parser, NXPC_DBG_CONTROL_SET_CHANNELS, 1U);
        if (startResponse.status != NXPC_DBG_CONTROL_STATUS_OK)
        {
        ...
        sendControl(port, 2U, NXPC_DBG_CONTROL_SET_CHANNELS);
        (void)waitForControlResponse(port, parser, NXPC_DBG_CONTROL_SET_CHANNELS, 2U);
        sendControl(port, 3U, NXPC_DBG_CONTROL_CLOSE);
        (void)waitForControlResponse(port, parser, NXPC_DBG_CONTROL_CLOSE, 3U);
