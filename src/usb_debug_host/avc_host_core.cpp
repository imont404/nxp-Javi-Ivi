...
constexpr uint32_t kPacketPayloadMaxBytes = (16u * 1024u) - AVC_DBG_PACKET_HEADER_BYTES;
constexpr uint32_t kFrameMaxBytes = 4u * 1024u * 1024u;
constexpr uint16_t kKnownPacketFlags = AVC_DBG_PACKET_FLAG_RESPONSE | AVC_DBG_PACKET_FLAG_MORE |
                                       AVC_DBG_PACKET_FLAG_PAYLOAD_CRC32 |
                                       AVC_DBG_PACKET_FLAG_DROPPED_BEFORE;
constexpr uint32_t kKnownChunkFlags = AVC_DBG_RUI_CHUNK_FRAME_START |
                                      AVC_DBG_RUI_CHUNK_FRAME_END |
                                ...
                                std::string &error)
{
    ...
    if (!requested_port.empty())
    {
        for (const SerialDevice &device : devices)
        {
            ...
            {
                if ((device.vid != kAvcUsbVid) || (device.pid != kAvcRuntimePid))
                {
                    error = requested_port + " is not an AVC runtime device (expected VID_1FC9/PID_0094)";
                    return false;
                }
                selected = device;
                return true;
    ...
    if (matches.empty())
    {
        error = "no AVC runtime CDC device found (VID_1FC9/PID_0094)";
        return false;
    }
    if (matches.size() != 1u)
    {
        std::ostringstream message;
        message << matches.size() << " AVC runtime devices found; select one with --port:";
...
struct StreamParser::Impl
{
    ...
    avc_dbg_stats_report_t latest_stats{};
    ...
    void parse_frame(const avc_dbg_packet_header_t &header,
                     const uint8_t *payload,
                     uint32_t payload_bytes)
    {
        if (payload_bytes <= sizeof(avc_dbg_rui_write_frame_buffer_raw_t))
        {
            set_error("frame payload is too small");
            return;
        }

        avc_dbg_rui_write_frame_buffer_raw_t chunk{};
        ...
        const bool valid = (chunk.total_frame_bytes > 0u) &&
                           ...
                           (chunk.pixel_format == AVC_DBG_PIXEL_FORMAT_RGB565_LE) &&
                           (chunk.buffer_id == 0u) &&
        ...
        if ((chunk.chunk_flags & AVC_DBG_RUI_CHUNK_FRAME_START) != 0u)
        {
            assembling = Frame{};
        ...
        if ((chunk.chunk_flags & AVC_DBG_RUI_CHUNK_FRAME_END) != 0u)
        {
    ...
    void parse_packet(const avc_dbg_packet_header_t &header,
                      const uint8_t *payload,
                      uint32_t payload_bytes)
    {
        if ((header.flags & AVC_DBG_PACKET_FLAG_DROPPED_BEFORE) != 0u)
        {
            ++count.dropped_before;
        }

        if (((header.flags & AVC_DBG_PACKET_FLAG_RESPONSE) != 0u) &&
            ((header.msg_id & 0xFFFFFF00u) == AVC_DBG_MSG_CLASS_CONTROL))
        {
        ...
        if (header.msg_id == AVC_DBG_RUI_WRITE_FRAME_BUFFER_RAW)
        {
            parse_frame(header, payload, payload_bytes);
            return;
        }

        if (header.msg_id == AVC_DBG_STATS_REPORT)
        {
        ...
        if (header.msg_id == AVC_DBG_LOG_TEXT)
        {
            if (payload_bytes < sizeof(avc_dbg_log_record_t))
            {
                set_error("log payload is too small");
                return;
            }
            avc_dbg_log_record_t wire{};
            ...
            if ((expected != payload_bytes) || (wire.level > AVC_DBG_LOG_LEVEL_ERROR))
            {
        ...
        if (header.msg_id == AVC_DBG_TELEMETRY_SCALAR)
        {
            if (payload_bytes < sizeof(avc_dbg_telemetry_scalar_t))
            {
                set_error("telemetry payload is too small");
                return;
            }
            avc_dbg_telemetry_scalar_t wire{};
            std::memcpy(&wire, payload, sizeof(wire));
            const uint32_t expected = static_cast<uint32_t>(sizeof(wire)) +
                                      wire.name_bytes + wire.units_bytes;
            const bool type_valid = (wire.value_type >= AVC_DBG_TELEMETRY_TYPE_I32) &&
                                    (wire.value_type <= AVC_DBG_TELEMETRY_TYPE_BOOL);
    ...
    void parse()
    {
        while ((buffer.size() - cursor) >= AVC_DBG_PACKET_HEADER_BYTES)
        {
            ...
            if ((buffer.size() - cursor) < AVC_DBG_PACKET_HEADER_BYTES)
            {
                return;
            }

            avc_dbg_packet_header_t header{};
            std::memcpy(&header, buffer.data() + cursor, sizeof(header));
            const bool header_valid = (header.magic == AVC_DBG_MAGIC) &&
                                      (header.version == AVC_DBG_VERSION) &&
                                      (header.header_bytes == AVC_DBG_PACKET_HEADER_BYTES) &&
            ...
            const size_t packet_bytes = AVC_DBG_PACKET_HEADER_BYTES +
                                        static_cast<size_t>(header.payload_length);
            if ((buffer.size() - cursor) < packet_bytes)
            {
                return;
            }
            const uint8_t *payload = buffer.data() + cursor + AVC_DBG_PACKET_HEADER_BYTES;
...
bool StreamParser::latest_stats(avc_dbg_stats_report_t &stats) const
{
    if (!impl_->have_stats)
    {
        return false;
...
avc_dbg_packet_header_t make_control_request(uint32_t request_sequence,
                                              uint32_t msg_id,
                                              uint32_t arg0,
                                              uint32_t arg1,
                                              uint32_t arg2)
...
bool send_control_request(SerialPort &port,
                          ...
                          std::string &error)
{
    const avc_dbg_packet_header_t request = make_control_request(request_sequence,
                                                                 msg_id,
                                                                 arg0,
                                                                 arg1,
                                                                 arg2);
...
{
    StreamParser parser;
    avc_dbg_control_hello_response_t hello_payload{};
    hello_payload.capability_flags = AVC_DBG_CAPABILITY_FRAMED_CONTROL |
                                     AVC_DBG_CAPABILITY_CAMERA_FRAMES |
    ...
    hello_payload.pixel_format = AVC_DBG_PIXEL_FORMAT_RGB565_LE;
    hello_payload.session_id = 0x12345678u;

    avc_dbg_packet_header_t hello_header = make_control_request(9u, AVC_DBG_CONTROL_HELLO);
    hello_header.flags = AVC_DBG_PACKET_FLAG_RESPONSE;
    hello_header.payload_length = sizeof(hello_payload);
    hello_header.arg0 = 9u;
    hello_header.arg1 = AVC_DBG_CONTROL_STATUS_OK;
    ...
    ControlResponse response;
    avc_dbg_control_hello_response_t decoded{};
    if (!parser.take_control_response(AVC_DBG_CONTROL_HELLO, 9u, response) ||
    ...
    while (offset < frame_bytes)
    {
        ...
        chunk.height = 200u;
        chunk.pixel_format = AVC_DBG_PIXEL_FORMAT_RGB565_LE;
        chunk.chunk_flags = ((offset == 0u) ? AVC_DBG_RUI_CHUNK_FRAME_START : 0u) |
                            (((offset + data_bytes) == frame_bytes) ? AVC_DBG_RUI_CHUNK_FRAME_END : 0u);
        avc_dbg_packet_header_t header{};
        header.magic = AVC_DBG_MAGIC;
        header.version = AVC_DBG_VERSION;
        header.header_bytes = AVC_DBG_PACKET_HEADER_BYTES;
        header.msg_id = AVC_DBG_RUI_WRITE_FRAME_BUFFER_RAW;
