...
void usage()
{
    std::cout << "AVC one-cable host protocol probe\n\n"
              << "usage:\n"
              << "  avc_tool.exe devices\n"
              << "  avc_tool.exe selftest\n"
...
{
    std::string error;
    const std::vector<avc::host::SerialDevice> devices = avc::host::list_serial_devices(error);
    if (!error.empty())
    {
    ...
    {
        for (const avc::host::SerialDevice &device : devices)
        {
    ...
    const std::vector<avc::host::HidDevice> rom_devices =
        avc::host::find_hid_devices(avc::host::kAvcUsbVid, avc::host::kMcxn947RomPid, error);
    if (!error.empty())
    {
        throw std::runtime_error(error);
    }
    ...
    for (const avc::host::HidDevice &device : rom_devices)
    {
...
int probe(const Options &options)
{
    ...
    uint32_t channels = AVC_DBG_CHANNEL_LOGS | AVC_DBG_CHANNEL_TELEMETRY;
    if (options.request_frame)
    {
        channels |= AVC_DBG_CHANNEL_FRAMES | AVC_DBG_CHANNEL_STATS;
    }
    if (!send_and_wait(port,
                       parser,
                       1u,
    ...
    const avc::host::ParserCounters &count = parser.counters();
    ...
    for (const avc::host::LogRecord &record : parser.logs())
    {
        std::cout << "log[" << record.category << "]=" << record.text << "\n";
    }
    for (const avc::host::TelemetrySample &sample : parser.telemetry())
    {
...
int program(const Options &options)
{
    ...
    {
        throw std::runtime_error("program requires --image <avc_core0.bin>");
    }

    std::string error;
    avc::host::FirmwareImage image;
    if (!avc::host::validate_firmware_image(options.image, image, error))
    {
        throw std::runtime_error(error);
    }
    ...
    const std::vector<avc::host::SerialDevice> runtime_devices =
        avc::host::find_serial_devices(avc::host::kAvcUsbVid,
                                       avc::host::kAvcRuntimePid,
                                       error);
    if (!error.empty())
    {
        throw std::runtime_error(error);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        {
            std::string error;
            if (!avc::host::run_core_self_test(error))
            {
                std::cerr << "selftest=failed: " << error << "\n";
                return 1;
            }
            if (!avc::host::run_programmer_self_test(error))
            {
                std::cerr << "selftest=failed: " << error << "\n";
                return 1;
            }
