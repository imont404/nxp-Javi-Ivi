/*
 * Bounded _sbrk for the NXP Cup firmware.
 *
 * Why this exists
 * ---------------
 ...
 * (see link/nxp_cup_core0_Debug_library.ld).
 *
 ...
 * ----------------------
 * Yes, indirectly. Nothing in NXP Cup calls malloc, but newlib's printf family
 * allocates for some conversions, and the overlay code formats floats
 * ("%.2f" in nxpc__update_overlay). Expected use is small and bounded; the
...
/* Provided by the linker script; see link/nxp_cup_core0_Debug.ld. */
