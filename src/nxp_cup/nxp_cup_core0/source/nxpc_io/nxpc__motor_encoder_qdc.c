...
static void nxpc__qdc_clear_flags(QDC_Type *base)
{
    base->CTRL = (uint16_t)((base->CTRL & (uint16_t)(~(uint16_t)NXPC_QDC_CTRL_W1C_FLAGS)) |
                            NXPC_QDC_CTRL_W1C_FLAGS);
    base->CTRL2 = (uint16_t)((base->CTRL2 & (uint16_t)(~(uint16_t)NXPC_QDC_CTRL2_W1C_FLAGS)) |
                             NXPC_QDC_CTRL2_W1C_FLAGS);
}

static void nxpc__qdc_zero(QDC_Type *base)
{
    base->UINIT = 0U;
    base->LINIT = 0U;
    base->CTRL = (uint16_t)((base->CTRL & (uint16_t)(~(uint16_t)NXPC_QDC_CTRL_W1C_FLAGS)) |
...
static void nxpc__qdc_init_block(QDC_Type *base, bool invert)
{
    /*
...
static void nxpc__motor_encoder_qdc_configure_pins(void)
{
...
{
    nxpc__motor_encoder_qdc_configure_pins();
    ...
    nxpc__qdc_init_block(QDC0, CONFIG__MOTOR_ENCODER_INVERT_M0 != 0);
    nxpc__qdc_init_block(QDC1, CONFIG__MOTOR_ENCODER_INVERT_M1 != 0);

    s_last_position[NXPC_MOTOR_ENCODER_M0] = nxpc__qdc_read_position(QDC0);
    s_last_position[NXPC_MOTOR_ENCODER_M1] = nxpc__qdc_read_position(QDC1);
                                   ...
                                   nxpc_motor_encoder_sample_t samples[NXPC_MOTOR_ENCODER_COUNT])
{
    ...
    for (uint32_t index = 0U; index < NXPC_MOTOR_ENCODER_COUNT; index++)
    {
        QDC_Type *base = s_qdc_base[index];
        uint32_t position = nxpc__qdc_read_position(base);
        ...
        samples[index].flags_ctrl = (uint16_t)(base->CTRL & NXPC_QDC_CTRL_W1C_FLAGS);
        samples[index].flags_ctrl2 = (uint16_t)(base->CTRL2 & NXPC_QDC_CTRL2_W1C_FLAGS);
